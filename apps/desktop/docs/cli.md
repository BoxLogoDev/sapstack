...
Terminal client for sapstack Desktop server. Connects over WebSocket (`ws://` or `wss://`) to a running headless server.
...
- For all other commands: a running sapstack Desktop headless server with URL and token

## Installation

```bash
# Clone the repository
git clone https://github.com/anthropics/sapstack-desktops.git
cd sapstack-desktops
...
|------|---------|---------|-------------|
| `--url <ws[s]://...>` | `SAPSTACK_DESKTOP_SERVER_URL` | — | Server WebSocket URL |
| `--token <secret>` | `SAPSTACK_DESKTOP_SERVER_TOKEN` | — | Authentication token |
| `--workspace <id>` | — | auto-detect | Workspace ID |
| `--timeout <ms>` | — | `10000` | Request timeout |
| `--tls-ca <path>` | `SAPSTACK_DESKTOP_TLS_CA` | — | Custom CA cert for self-signed TLS |
...
The `--tls-ca` flag sets `NODE_EXTRA_CA_CERTS` before connecting. You can also set `SAPSTACK_DESKTOP_TLS_CA` in your environment.
...
| `AUTH_FAILED` | Wrong token | Check `SAPSTACK_DESKTOP_SERVER_TOKEN` matches server |
