...
import type { StoredAttachment, ContentBadge } from '@sapstack-desktop/core'
import { normalizePath } from '@sapstack-desktop/core/utils'
...
function InlineFileBadge({
  ...
  // Strip .sapstack-desktop workspace/session path prefix for cleaner tooltip display
  // e.g. "/Users/.../workspaces/{id}/sessions/{id}/plans/foo.md" → "plans/foo.md"
  const rawPath = badge.filePath || badge.label
  const tooltipPath = normalizePath(rawPath).replace(/^.*\.sapstack-desktop\/workspaces\/[^/]+\/(sessions\/[^/]+\/)?/, '')
