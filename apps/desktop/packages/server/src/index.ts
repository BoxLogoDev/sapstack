#!/usr/bin/env bun
/**
 * @sapstack-desktop/server — standalone headless sapstack Desktop server.
 *
 * Usage:
 *   SAPSTACK_DESKTOP_SERVER_TOKEN=<secret> bun run packages/server/src/index.ts
 *
 * Environment:
 *   SAPSTACK_DESKTOP_SERVER_TOKEN         — required bearer token for client auth
 *   SAPSTACK_DESKTOP_RPC_HOST             — bind address (default: 127.0.0.1)
 *   SAPSTACK_DESKTOP_RPC_PORT             — bind port (default: 9100)
 *   SAPSTACK_DESKTOP_RPC_TLS_CERT         — path to PEM certificate file (enables TLS/wss)
 *   SAPSTACK_DESKTOP_RPC_TLS_KEY          — path to PEM private key file (required with cert)
 *   SAPSTACK_DESKTOP_RPC_TLS_CA           — path to PEM CA chain file (optional)
 *   SAPSTACK_DESKTOP_APP_ROOT             — app root path (default: cwd)
 *   SAPSTACK_DESKTOP_RESOURCES_PATH       — resources path (default: cwd/resources)
 *   SAPSTACK_DESKTOP_IS_PACKAGED          — 'true' for production (default: false)
 *   SAPSTACK_DESKTOP_VERSION              — app version (default: 0.0.0-dev)
 *   SAPSTACK_DESKTOP_DEBUG                — 'true' for debug logging
 *   SAPSTACK_DESKTOP_WEBUI_DIR            — path to built web UI assets (enables web UI on RPC port)
 *   SAPSTACK_DESKTOP_WEBUI_PASSWORD       — optional shorter password for web login (falls back to SAPSTACK_DESKTOP_SERVER_TOKEN)
 *   SAPSTACK_DESKTOP_WEBUI_SECURE_COOKIE  — optional true/false override for the session cookie Secure flag
 *   SAPSTACK_DESKTOP_WEBUI_WS_URL         — optional browser-facing ws:// or wss:// URL returned by /api/config
 *   SAPSTACK_DESKTOP_MESSAGING_WA_WORKER  — absolute path to worker.cjs (default: packages/messaging-whatsapp-worker/dist/worker.cjs)
 *   SAPSTACK_DESKTOP_MESSAGING_NODE_BIN   — Node binary used to spawn the WhatsApp worker (default: node)
 */
...
import { enableDebug } from '@sapstack-desktop/shared/utils/debug'
import { bootstrapServer, startHealthHttpServer, generateServerToken } from '@sapstack-desktop/server-core/bootstrap'
import { validateSession, createWebuiHandler, nodeHttpAdapter } from '@sapstack-desktop/server-core/webui'
import type { WebuiHandler } from '@sapstack-desktop/server-core/webui'
import { getCredentialManager } from '@sapstack-desktop/shared/credentials'
import { getWorkspaces } from '@sapstack-desktop/shared/config'
...
import type { WsRpcTlsOptions } from '@sapstack-desktop/server-core/transport'
...
import { initModelRefreshService, setFetcherPlatform } from '@sapstack-desktop/server-core/model-fetchers'
import { setSearchPlatform, setImageProcessor } from '@sapstack-desktop/server-core/services'
import type { HandlerDeps } from '@sapstack-desktop/server-core/handlers'

process.env.SAPSTACK_DESKTOP_IS_PACKAGED ??= 'false'
...
if (process.env.SAPSTACK_DESKTOP_DEBUG === 'true' || process.env.SAPSTACK_DESKTOP_DEBUG === '1') {
  enableDebug()
}
...
// In packaged mode, use SAPSTACK_DESKTOP_BUNDLED_ASSETS_ROOT env or cwd.
const bundledAssetsRoot = process.env.SAPSTACK_DESKTOP_BUNDLED_ASSETS_ROOT
  ?? join(import.meta.dir, '..', '..', '..', '..')

// TLS configuration — when cert + key paths are provided, server listens on wss://
let tls: WsRpcTlsOptions | undefined
const tlsCertPath = process.env.SAPSTACK_DESKTOP_RPC_TLS_CERT
const tlsKeyPath = process.env.SAPSTACK_DESKTOP_RPC_TLS_KEY
if (tlsCertPath || tlsKeyPath) {
  if (!tlsCertPath || !tlsKeyPath) {
    console.error('TLS requires both SAPSTACK_DESKTOP_RPC_TLS_CERT and SAPSTACK_DESKTOP_RPC_TLS_KEY.')
    process.exit(1)
  }
  tls = {
    cert: readFileSync(tlsCertPath),
    key: readFileSync(tlsKeyPath),
    ...(process.env.SAPSTACK_DESKTOP_RPC_TLS_CA ? { ca: readFileSync(process.env.SAPSTACK_DESKTOP_RPC_TLS_CA) } : {}),
  }
}

// Web UI configuration
const webuiDir = process.env.SAPSTACK_DESKTOP_WEBUI_DIR || undefined
const webuiEnabled = webuiDir && existsSync(webuiDir)
const webuiSecureCookies = parseOptionalBooleanEnv('SAPSTACK_DESKTOP_WEBUI_SECURE_COOKIE', process.env.SAPSTACK_DESKTOP_WEBUI_SECURE_COOKIE)
const webuiWsUrl = parseOptionalWebSocketUrl('SAPSTACK_DESKTOP_WEBUI_WS_URL', process.env.SAPSTACK_DESKTOP_WEBUI_WS_URL)
const serverToken = process.env.SAPSTACK_DESKTOP_SERVER_TOKEN
...
if (webuiEnabled && serverToken) {
  const rpcPort = parseInt(process.env.SAPSTACK_DESKTOP_RPC_PORT ?? '9100', 10)
  const rpcProtocol = tls ? 'wss' as const : 'ws' as const

  webuiHandler = createWebuiHandler({
    webuiDir: webuiDir!,
    secret: serverToken,
    password: process.env.SAPSTACK_DESKTOP_WEBUI_PASSWORD || undefined,
...
const waWorkerEntry = process.env.SAPSTACK_DESKTOP_MESSAGING_WA_WORKER
  ?? join(bundledAssetsRoot, 'packages', 'messaging-whatsapp-worker', 'dist', 'worker.cjs')
const waNodeBin = process.env.SAPSTACK_DESKTOP_MESSAGING_NODE_BIN ?? 'node'
...
const instance = await (async () => {
  try {
    return await bootstrapServer<SessionManager, HandlerDeps>({
      bundledAssetsRoot,
      serverVersion: process.env.SAPSTACK_DESKTOP_VERSION ?? packageVersion,
      tls,
...
// ---------------------------------------------------------------------------
// SAPSTACK_DESKTOP_DISABLE_MESSAGING lets a dev/test server share a config dir with a live app
// without both processes fighting over the same Telegram/WhatsApp connections (409s).
const messagingDisabled = process.env.SAPSTACK_DESKTOP_DISABLE_MESSAGING === 'true' || process.env.SAPSTACK_DESKTOP_DISABLE_MESSAGING === '1'
if (messagingHandle !== null && !messagingDisabled) {
  ...
  console.log('[messaging] Disabled via SAPSTACK_DESKTOP_DISABLE_MESSAGING — skipping workspace messaging init')
}

// Wire up the lazy health check now that the session manager is ready
if (webuiHandler) {
  const { getHealthCheck } = await import('@sapstack-desktop/server-core/handlers/rpc/server')
  ...
  const { getSourceCredentialManager, loadWorkspaceSources } = await import('@sapstack-desktop/shared/sources')
  const { getWorkspaceByNameOrId } = await import('@sapstack-desktop/shared/config')
  const { pushTyped } = await import('@sapstack-desktop/server-core/transport')
  const { RPC_CHANNELS } = await import('@sapstack-desktop/shared/protocol')
...
// Start HTTP health endpoint if SAPSTACK_DESKTOP_HEALTH_PORT is set
const healthPort = parseInt(process.env.SAPSTACK_DESKTOP_HEALTH_PORT ?? '0', 10)
...
if (webuiHandler) {
  console.log(`SAPSTACK_DESKTOP_WEBUI_URL=${serverProto}://0.0.0.0:${instance.port}`)
}
...
if (!isLocalBind && instance.protocol === 'ws') {
  if (process.argv.includes('--allow-insecure-bind')) {
    console.warn(
      ...
      '   Set SAPSTACK_DESKTOP_RPC_TLS_CERT and SAPSTACK_DESKTOP_RPC_TLS_KEY to enable wss://.\n'
    )
  } else {
    console.error(
      ...
      '   Options:\n' +
      '     1. Set SAPSTACK_DESKTOP_RPC_TLS_CERT and SAPSTACK_DESKTOP_RPC_TLS_KEY to enable wss://\n' +
