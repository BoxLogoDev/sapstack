...
export interface ResolveScriptRuntimeContext {
  /**
   * Whether host app is packaged. Defaults to SAPSTACK_DESKTOP_IS_PACKAGED=1.
...
function inferPackagedMode(ctx?: ResolveScriptRuntimeContext): boolean {
  ...
  return process.env.SAPSTACK_DESKTOP_IS_PACKAGED === '1';
}
...
function resolveResourcesBase(ctx?: ResolveScriptRuntimeContext): string | null {
  const explicit = ctx?.resourcesBasePath || process.env.SAPSTACK_DESKTOP_RESOURCES_BASE;
...
function resolveAppRoot(ctx?: ResolveScriptRuntimeContext): string | null {
  const explicit = ctx?.appRootPath || process.env.SAPSTACK_DESKTOP_APP_ROOT;
...
function validatePackagedEnvRuntime(command: string, label: string): string {
  ...
  if (!isAbsolute(command) && !hasPathSeparator) {
    throw new Error(
      ...
      'Packaged builds do not allow PATH-based runtime resolution. Configure an absolute SAPSTACK_DESKTOP_* path or ship a bundled runtime.'
    );
  }
  ...
  if (!existsSync(resolvedCommand)) {
    throw new Error(
      ...
      'Configure a valid absolute SAPSTACK_DESKTOP_* path or ship a bundled runtime.'
    );
  }
 ...
 *
 * Resolution order:
 * - env override (SAPSTACK_DESKTOP_UV / SAPSTACK_DESKTOP_NODE / SAPSTACK_DESKTOP_BUN)
...
export function resolveScriptRuntime(
  ...
  if (language === 'python3') {
    if (process.env.SAPSTACK_DESKTOP_UV) {
      const cmd = isPackaged
        ? validatePackagedEnvRuntime(process.env.SAPSTACK_DESKTOP_UV, 'Python/uv')
        : process.env.SAPSTACK_DESKTOP_UV;
    ...
    throw new Error(
      isPackaged
        ...
        : 'Python runtime unavailable: uv was not found. Configure SAPSTACK_DESKTOP_UV or install uv on PATH.'
    );
  }

  if (language === 'node') {
    if (process.env.SAPSTACK_DESKTOP_NODE) {
      const cmd = isPackaged
        ? validatePackagedEnvRuntime(process.env.SAPSTACK_DESKTOP_NODE, 'Node')
        : process.env.SAPSTACK_DESKTOP_NODE;
    ...
    throw new Error(
      isPackaged
        ...
        : 'Node runtime unavailable: configure SAPSTACK_DESKTOP_NODE or install node on PATH.'
    );
  }

  if (process.env.SAPSTACK_DESKTOP_BUN) {
    const cmd = isPackaged
      ? validatePackagedEnvRuntime(process.env.SAPSTACK_DESKTOP_BUN, 'Bun')
      : process.env.SAPSTACK_DESKTOP_BUN;
    return { command: cmd, argsPrefix: [], source: 'env' };
  }
  ...
  throw new Error(
    isPackaged
      ...
      : 'Bun runtime unavailable: configure SAPSTACK_DESKTOP_BUN or install bun on PATH.'
  );
}
