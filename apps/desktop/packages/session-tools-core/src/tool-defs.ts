...
export const UpdatePreferencesSchema = z.object({
  ...
  includeCoAuthoredBy: z.boolean().optional().describe("Whether to include 'Co-Authored-By: sapstack Desktop' trailer on git commits. Defaults to true."),
});
...
- Do NOT include any text or tool calls after SubmitPlan - they will not be executed`,
  ...
  config_validate: `Validate sapstack Desktop configuration files.
...
Only use 'attachments' for existing file paths on disk — the tool reads them automatically.`,
  ...
  send_developer_feedback: `Send freeform feedback to the sapstack Desktop development team.
...
IMPORTANT: never move a task into a closed status (such as "done" or "cancelled") yourself — closing a task is the user's decision, made on the board. You may prepare and hand off work by setting an open status like "needs-review"; the user reviews and closes it. Closed-status calls are rejected.`,
  ...
  create_task: `Create a sapstack Desktop Task on the kanban board — writes tasks/<slug>/task.yaml and creates its orchestrator session. CREATION ONLY: the task lands in "todo" and is NOT run; starting it is the user's (or an automation's) decision.
