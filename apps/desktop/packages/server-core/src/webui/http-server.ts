...
import { generateCallbackPage } from '@sapstack-desktop/shared/auth'
...
export interface WebuiHandlerOptions {
  ...
  webuiDir: string
  /** Secret used to sign JWTs — typically SAPSTACK_DESKTOP_SERVER_TOKEN. */
  secret: string
...
export function createWebuiHandler(options: WebuiHandlerOptions): WebuiHandler {
  ...
  async function fetch(req: Request): Promise<Response> {
    ...
    if (path === '/api/config/workspaces' && req.method === 'GET') {
      ...
      const { getActiveWorkspace } = await import('@sapstack-desktop/shared/config/storage')
