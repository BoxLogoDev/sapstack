...
function createConsoleLogger(): Logger {
  ...
  return {
    ...
    debug: (...args) => {
      if (process.env.SAPSTACK_DESKTOP_DEBUG === 'true' || process.env.SAPSTACK_DESKTOP_IS_PACKAGED !== 'true') {
 ...
 * - SAPSTACK_DESKTOP_APP_ROOT — override appRootPath (default: cwd)
 * - SAPSTACK_DESKTOP_RESOURCES_PATH — override resourcesPath (default: cwd/resources)
 * - SAPSTACK_DESKTOP_IS_PACKAGED — 'true' for production (default: false)
 * - SAPSTACK_DESKTOP_VERSION — app version string (default: '0.0.0-dev')
 * - SAPSTACK_DESKTOP_DEBUG — 'true' to enable debug logging
 */
export function createHeadlessPlatform(options?: { appVersion?: string }): PlatformServices {
  const logger = createConsoleLogger()
  const isDebugMode = process.env.SAPSTACK_DESKTOP_DEBUG === 'true' || process.env.SAPSTACK_DESKTOP_IS_PACKAGED !== 'true'

  return {
    appRootPath: process.env.SAPSTACK_DESKTOP_APP_ROOT || process.cwd(),
    resourcesPath: process.env.SAPSTACK_DESKTOP_RESOURCES_PATH || join(process.cwd(), 'resources'),
    isPackaged: process.env.SAPSTACK_DESKTOP_IS_PACKAGED === 'true',
    appVersion: process.env.SAPSTACK_DESKTOP_VERSION || options?.appVersion || '0.0.0-dev',
