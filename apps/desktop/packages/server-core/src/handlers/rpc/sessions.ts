...
import { RPC_CHANNELS, type FileAttachment, type SendMessageOptions, type SessionEvent } from '@sapstack-desktop/shared/protocol'
import type { StoredAttachment } from '@sapstack-desktop/core/types'
import { getWorkspaceByNameOrId } from '@sapstack-desktop/shared/config'
import { perf } from '@sapstack-desktop/shared/utils'
import { isValidThinkingLevel, THINKING_LEVEL_IDS } from '@sapstack-desktop/shared/agent/thinking-levels'
...
async function scanSessionDirectory(dirPath: string): Promise<import('@sapstack-desktop/shared/protocol').SessionFile[]> {
  const { readdir, stat } = await import('fs/promises')
  const entries = await readdir(dirPath, { withFileTypes: true })
  const files: import('@sapstack-desktop/shared/protocol').SessionFile[] = []
...
export function registerSessionsHandlers(server: RpcServer, deps: HandlerDeps): void {
  ...
  server.handle(RPC_CHANNELS.sessions.CREATE, async (_ctx, workspaceId: string, options?: import('@sapstack-desktop/shared/protocol').CreateSessionOptions) => {
  ...
  // Returns true if the response was delivered, false if agent/session is gone
  server.handle(RPC_CHANNELS.sessions.RESPOND_TO_CREDENTIAL, async (_ctx, sessionId: string, requestId: string, response: import('@sapstack-desktop/shared/protocol').CredentialResponse) => {
  ...
  server.handle(RPC_CHANNELS.sessions.COMMAND, async (
    _ctx,
    sessionId: string,
    command: import('@sapstack-desktop/shared/protocol').SessionCommand
  ) => {
    switch (command.type) {
      case 'flag':
  ...
  server.handle(RPC_CHANNELS.sessions.SEARCH_CONTENT, async (_ctx, workspaceId: string, query: string, searchId?: string) => {
    ...
    const { searchSessions } = await import('@sapstack-desktop/server-core/services')
    const { getWorkspaceSessionsPath } = await import('@sapstack-desktop/shared/workspaces')
  ...
  const importHandler = async (_ctx: any, targetWorkspaceId: string, bundle: unknown, mode: string) => {
    ...
    return sessionManager.importSession(targetWorkspaceId, bundle as import('@sapstack-desktop/shared/sessions').SessionBundle, mode)
  }
  ...
  server.handle(RPC_CHANNELS.sessions.IMPORT_REMOTE_TRANSFER, async (_ctx, targetWorkspaceId: string, payload: import('@sapstack-desktop/shared/protocol').RemoteSessionTransferPayload) => {
    await sessionManager.waitForInit()
    if (!targetWorkspaceId || typeof targetWorkspaceId !== 'string') throw new Error('targetWorkspaceId is required')
