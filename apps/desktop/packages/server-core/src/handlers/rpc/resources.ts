...
import { getCredentialManager, SOURCE_CREDENTIAL_TYPES } from '@sapstack-desktop/shared/credentials'
...
} from '@sapstack-desktop/shared/resources'
...
export function registerResourcesHandlers(server: RpcServer, deps: HandlerDeps): void {
  ...
  server.handle(
    ...
    async (_ctx, workspaceId: string, options: ExportResourcesOptions) => {
      ...
      const { exportResources } = await import('@sapstack-desktop/shared/resources')
  ...
  server.handle(
    ...
    async (_ctx, workspaceId: string, bundle: ResourceBundle, mode: ResourceImportMode) => {
      ...
      const { importResources } = await import('@sapstack-desktop/shared/resources')
