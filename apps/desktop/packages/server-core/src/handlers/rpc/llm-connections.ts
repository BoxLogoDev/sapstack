import { RPC_CHANNELS, type LlmConnectionSetup } from '@sapstack-desktop/shared/protocol'
import { getLlmConnections, getLlmConnection, addLlmConnection, updateLlmConnection, deleteLlmConnection, getDefaultLlmConnection, setDefaultLlmConnection, touchLlmConnection, isCompatProvider, isAnthropicProvider, getDefaultModelsForConnection, getDefaultModelForConnection, type LlmConnection, type LlmConnectionWithStatus, toBedrockNativeId, deriveBedrockRegionPrefix } from '@sapstack-desktop/shared/config'
import { getCredentialManager } from '@sapstack-desktop/shared/credentials'
import { setSetupDeferred } from '@sapstack-desktop/shared/config/storage'
import {
  resolveSetupTestConnectionHint,
  testBackendConnection,
  validateStoredBackendConnection,
} from '@sapstack-desktop/shared/agent/backend'
import { getModelRefreshService } from '@sapstack-desktop/server-core/model-fetchers'
import { parseTestConnectionError, createBuiltInConnection, validateModelList, piAuthProviderDisplayName, validateSetupTestInput, setupTestRequiresApiKey, resolveCustomEndpointSetup } from '@sapstack-desktop/server-core/domain'
import { getWorkspaceOrThrow, buildBackendHostRuntimeContext } from '@sapstack-desktop/server-core/handlers'
import { pushTyped, type RpcServer } from '@sapstack-desktop/server-core/transport'
import type { HandlerDeps } from '../handler-deps'
import { randomUUID } from 'node:crypto'
import { CLIENT_OPEN_EXTERNAL } from '@sapstack-desktop/server-core/transport'
...
export function registerLlmConnectionsHandlers(server: RpcServer, deps: HandlerDeps): void {
  ...
  server.handle(RPC_CHANNELS.settings.SETUP_LLM_CONNECTION, async (_ctx, setup: LlmConnectionSetup): Promise<{ success: boolean; error?: string }> => {
    try {
      ...
      if (setup.piAuthProvider && !isCustomEndpointCompat) {
        ...
        // Update connection name to show the actual provider (e.g. "sapstack Desktop Agent Runtime (Google AI Studio)")
        const providerName = piAuthProviderDisplayName(setup.piAuthProvider)
        if (providerName) {
          updates.name = `sapstack Desktop Agent Runtime (${providerName})`
        }
  ...
  server.handle(RPC_CHANNELS.settings.TEST_LLM_CONNECTION_SETUP, async (_ctx, params: import('@sapstack-desktop/shared/protocol').TestLlmConnectionParams): Promise<import('@sapstack-desktop/shared/protocol').TestLlmConnectionResult> => {
  ...
  server.handle(RPC_CHANNELS.pi.GET_API_KEY_PROVIDERS, async () => {
    const { getPiApiKeyProviders } = await import('@sapstack-desktop/shared/config')
    return getPiApiKeyProviders()
  })

  server.handle(RPC_CHANNELS.pi.GET_PROVIDER_BASE_URL, async (_ctx, provider: string) => {
    const { getPiProviderBaseUrl } = await import('@sapstack-desktop/shared/config')
  ...
  server.handle(RPC_CHANNELS.llmConnections.TEST, async (_ctx, slug: string): Promise<{ success: boolean; error?: string }> => {
    try {
      ...
      const { parseValidationError } = await import('@sapstack-desktop/shared/config')
  ...
  server.handle(RPC_CHANNELS.chatgpt.START_OAUTH, async (ctx, connectionSlug: string): Promise<{
    ...
    const { prepareChatGptOAuth } = await import('@sapstack-desktop/shared/auth')
  ...
  server.handle(RPC_CHANNELS.chatgpt.COMPLETE_OAUTH, async (ctx, args: {
    ...
    try {
      const { exchangeChatGptTokens } = await import('@sapstack-desktop/shared/auth')
      const credentialManager = getCredentialManager()
