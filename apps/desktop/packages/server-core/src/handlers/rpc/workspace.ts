...
import { getWorkspaceByNameOrId, addWorkspace, setActiveWorkspace, updateWorkspaceRemoteServer } from '@sapstack-desktop/shared/config'
import { perf } from '@sapstack-desktop/shared/utils'
...
export function registerWorkspaceCoreHandlers(server: RpcServer, deps: HandlerDeps): void {
  ...
  server.handle(RPC_CHANNELS.workspaces.CHECK_SLUG, async (_ctx, slug: string) => {
    const defaultWorkspacesDir = join(homedir(), '.sapstack-desktop', 'workspaces')
  ...
  server.handle(RPC_CHANNELS.theme.GET_APP, async () => {
    const { loadAppTheme } = await import('@sapstack-desktop/shared/config/storage')
    return loadAppTheme()
  })

  // Preset themes (app-level)
  server.handle(RPC_CHANNELS.theme.GET_PRESETS, async () => {
    const { loadPresetThemes } = await import('@sapstack-desktop/shared/config/storage')
    return loadPresetThemes()
  })

  server.handle(RPC_CHANNELS.theme.LOAD_PRESET, async (_ctx, themeId: string) => {
    const { loadPresetTheme } = await import('@sapstack-desktop/shared/config/storage')
    return loadPresetTheme(themeId)
  })

  server.handle(RPC_CHANNELS.theme.GET_COLOR_THEME, async () => {
    const { getColorTheme } = await import('@sapstack-desktop/shared/config/storage')
    return getColorTheme()
  })

  server.handle(RPC_CHANNELS.theme.SET_COLOR_THEME, async (_ctx, themeId: string) => {
    const { setColorTheme } = await import('@sapstack-desktop/shared/config/storage')
  ...
  server.handle(RPC_CHANNELS.theme.GET_WORKSPACE_COLOR_THEME, async (_ctx, workspaceId: string) => {
    const { getWorkspaces } = await import('@sapstack-desktop/shared/config/storage')
    const { getWorkspaceColorTheme } = await import('@sapstack-desktop/shared/workspaces/storage')
  ...
  server.handle(RPC_CHANNELS.theme.SET_WORKSPACE_COLOR_THEME, async (_ctx, workspaceId: string, themeId: string | null) => {
    const { getWorkspaces } = await import('@sapstack-desktop/shared/config/storage')
    const { setWorkspaceColorTheme } = await import('@sapstack-desktop/shared/workspaces/storage')
    const workspaces = getWorkspaces()
    const workspace = workspaces.find(w => w.id === workspaceId)
    if (!workspace) return
  ...
  server.handle(RPC_CHANNELS.views.LIST, async (_ctx, workspaceId: string) => {
    ...
    const { listViews } = await import('@sapstack-desktop/shared/views/storage')
    return listViews(workspace.rootPath)
  })

  // Save views (replaces full array)
  server.handle(RPC_CHANNELS.views.SAVE, async (_ctx, workspaceId: string, views: import('@sapstack-desktop/shared/views').ViewConfig[]) => {
    const workspace = getWorkspaceByNameOrId(workspaceId)
    if (!workspace) throw new Error('Workspace not found')

    const { saveViews } = await import('@sapstack-desktop/shared/views/storage')
  ...
  server.handle(RPC_CHANNELS.toolIcons.GET_MAPPINGS, async () => {
    const { getToolIconsDir } = await import('@sapstack-desktop/shared/config/storage')
    const { loadToolIconConfig } = await import('@sapstack-desktop/shared/utils/cli-icon-resolver')
    const { encodeIconToDataUrl } = await import('@sapstack-desktop/shared/utils/icon-encoder')
  ...
  server.handle(RPC_CHANNELS.logo.GET_URL, async (_ctx, serviceUrl: string, provider?: string) => {
    const { getLogoUrl } = await import('@sapstack-desktop/shared/utils/logo')
