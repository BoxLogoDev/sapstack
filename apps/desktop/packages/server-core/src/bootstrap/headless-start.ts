...
import { ensureConfigDir, loadStoredConfig, saveConfig } from '@sapstack-desktop/shared/config'
import { CONFIG_DIR } from '@sapstack-desktop/shared/config/paths'
        ...
        if (lock.pid === process.pid) {
          ...
          if (isLockFromPreviousBoot(lock.startedAt)) {
            ...
            throw new Error(
              ...
              `To run a parallel instance (e.g. for dev), set SAPSTACK_DESKTOP_CONFIG_DIR to a different path.`
            )
          }
        } else {
...
export async function bootstrapServer<TSessionManager, THandlerDeps>(
  ...
  const serverToken = options.serverToken ?? process.env.SAPSTACK_DESKTOP_SERVER_TOKEN
  if (!serverToken) {
    throw new Error('Server token is required. Pass options.serverToken or set SAPSTACK_DESKTOP_SERVER_TOKEN.')
  }
  ...
  const bundledAssetsRoot = options.bundledAssetsRoot
    ?? process.env.SAPSTACK_DESKTOP_BUNDLED_ASSETS_ROOT
    ?? process.cwd()
  ...
  const rpcHost = options.rpcHost ?? process.env.SAPSTACK_DESKTOP_RPC_HOST ?? '127.0.0.1'
  const rpcPortRaw = options.rpcPort ?? parseInt(process.env.SAPSTACK_DESKTOP_RPC_PORT ?? '9100', 10)
  ...
  platform.logger.info(`sapstack Desktop server listening on ${wsServer.protocol}://${rpcHost}:${wsServer.port}`)
