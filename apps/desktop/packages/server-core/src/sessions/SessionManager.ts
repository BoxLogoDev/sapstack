import type { EventSink, RpcServer } from '@sapstack-desktop/server-core/transport'
import { CLIENT_BROWSER_INVOKE } from '@sapstack-desktop/server-core/transport'
import type { ISessionManager, IBrowserPaneManager, ExecutePromptAutomationInput } from '@sapstack-desktop/server-core/handlers'
import { RemoteBrowserPaneManager } from './RemoteBrowserPaneManager'
import { validateFilePath, getWorkspaceAllowedDirs } from '@sapstack-desktop/server-core/handlers'
import { createScopedLogger, CONSOLE_LOGGER, type PlatformServices, type Logger } from '@sapstack-desktop/server-core/runtime'
import { basename, dirname, join } from 'path'
import { existsSync } from 'fs'
import { readFile, writeFile, mkdir } from 'fs/promises'
import { randomUUID } from 'node:crypto'
import { type AgentEvent, setPermissionMode, hydratePreviousPermissionMode, getPermissionModeDiagnostics, type PermissionMode, unregisterSessionScopedToolCallbacks, mergeSessionScopedToolCallbacks, AbortReason, type AuthRequest, type AuthResult, type CredentialAuthRequest, type BrowserPaneFns, generateConversationSummary, resolveKeepBackgroundTasksAlive } from '@sapstack-desktop/shared/agent'
import {
  ...
  type PostInitResult,
} from '@sapstack-desktop/shared/agent/backend'
import { getLlmConnection, getLlmConnections, getDefaultLlmConnection, getDefaultThinkingLevel, resetManagedAnthropicAuthEnvVars, resolveMidStreamBehavior, getPersistedUiLanguage, resolveTitleLanguageName } from '@sapstack-desktop/shared/config'
import type { MidStreamBehavior } from '@sapstack-desktop/shared/config'
import { PrivilegedExecutionBroker } from '@sapstack-desktop/server-core/services'
import { isValidWorkingDirectory } from '../utils/path-validation'
import { InitGate } from '@sapstack-desktop/server-core/domain'
import { i18n } from '@sapstack-desktop/shared/i18n'
import {
  getWorkspaces,
...
import type { ActiveSessionInfo, SessionProcessingStatus } from '@sapstack-desktop/core/types'
import { loadWorkspaceConfig } from '@sapstack-desktop/shared/workspaces'
import {
...
import { listTaskSlugs, parseTaskSpec, uniqueTaskSlug } from '@sapstack-desktop/shared/tasks'
import { createTaskFromSpec, resolveCreateTaskProjectId } from '../tasks'
import { ConfigWatcher, type ConfigWatcherCallbacks } from '@sapstack-desktop/shared/config'
import { getValidClaudeOAuthToken } from '@sapstack-desktop/shared/auth'
import { resolveAuthEnvVars } from '@sapstack-desktop/shared/config'
import { toolMetadataStore, getLastApiError } from '@sapstack-desktop/shared/interceptor'
import { isParentTaskTool } from '@sapstack-desktop/shared/utils/toolNames'
import { restoreFiles } from '@sapstack-desktop/shared/utils/bundle-files'
import { getCredentialManager } from '@sapstack-desktop/shared/credentials'
import { CraftMcpClient, McpClientPool, McpPoolServer } from '@sapstack-desktop/shared/mcp'
import { type Session, type SessionEvent, type FileAttachment, type SendMessageOptions, type UnreadSummary, type RemoteSessionTransferPayload, type ImportRemoteSessionTransferResult, RPC_CHANNELS, generateMessageId } from '@sapstack-desktop/shared/protocol'
import { messageToStored, storedToMessage, type Message, type StoredAttachment, type ToolDisplayMeta, type TokenUsage } from '@sapstack-desktop/core/types'
import { formatPathsToRelative, formatToolInputPaths, perf, encodeIconToDataUrlAsync, getEmojiIcon, resetSummarizationClient, resolveToolIcon, readFileAttachment, selectSpreadMessages, normalizePath } from '@sapstack-desktop/shared/utils'
import { loadAllSkills, loadSkillBySlug, invalidateSkillsCache, type LoadedSkill } from '@sapstack-desktop/shared/skills'
import { invalidateContextFileCache } from '@sapstack-desktop/shared/prompts/system'
import { getToolIconsDir, getMiniModel } from '@sapstack-desktop/shared/config'
import { getDefaultSummarizationModel } from '@sapstack-desktop/shared/config/models'
import type { SummarizeCallback } from '@sapstack-desktop/shared/sources'
import { type ThinkingLevel, DEFAULT_THINKING_LEVEL, normalizeThinkingLevel } from '@sapstack-desktop/shared/agent/thinking-levels'
import { evaluateAutoLabels } from '@sapstack-desktop/shared/labels/auto'
import { listLabels, loadLabelConfig } from '@sapstack-desktop/shared/labels/storage'
import { extractLabelId, resolveSessionLabels, findTaskItemLabelId } from '@sapstack-desktop/shared/labels'
import { ensureLabelsExist, ensureTaskItemLabel } from '@sapstack-desktop/shared/labels/crud'
import { loadStatusConfig } from '@sapstack-desktop/shared/statuses/storage'
import { AutomationSystem, createPromptHistoryEntry, appendAutomationHistoryEntry, type AutomationSystemMetadataSnapshot } from '@sapstack-desktop/shared/automations'
import { buildBackendRuntimeSignature, buildRestartRequiredSignature, filterAttachmentsForModelInput } from './runtime-config'

// Import from server-core domain utilities
import { sanitizeForTitle, shouldActivateBrowserOverlay, normalizeBrowserToolName, rollbackFailedBranchCreation, releaseBrowserOwnershipOnForcedStop } from '@sapstack-desktop/server-core/domain'
import { resizeImageForAPI, resizeIconBuffer } from '@sapstack-desktop/server-core/services'
...
// validateSpawnAttachmentPath removed — use shared validateFilePath from @sapstack-desktop/server-core/handlers
 ...
 * what the LLM sees. See sapstack-desktops-oss#782.
 */
...
async function applyBridgeUpdates(
  ...
  mcpServers: Record<string, import('@sapstack-desktop/shared/agent/backend').SdkMcpServerConfig>,
  sessionId: string,
...
async function getBrowserToolIconDataUrl(): Promise<string | undefined> {
  ...
  try {
    const iconCandidates = [
      ...
      // Dev fallback (before sync to ~/.sapstack-desktop/tool-icons)
...
async function resolveToolDisplayMeta(
  ...
  // and resolves their brand icon from ~/.sapstack-desktop/tool-icons/
...
interface ManagedSession {
   ...
   * so the SDK's session-manager has updated its leaf — see sapstack-desktops-oss#782).
   * Capped at PI_SDK_MESSAGE_ID_CACHE_LIMIT to bound memory in long sessions.
   */
  piSdkMessageToCraftMessage?: Map<string, string>
  // Source-activation auto-retry (sapstack-desktops-oss#804). When a source activates
...
export class SessionManager implements ISessionManager {
  ...
  private pendingCredentialResolvers: Map<string, (response: import('@sapstack-desktop/shared/protocol').CredentialResponse) => void> = new Map()
  ...
  /**
   * WS2 keep-alive flag (default ON, opt-out via `SAPSTACK_DESKTOP_KEEP_BG_AGENTS_ALIVE=0`).
  ...
  setupConfigWatcher(workspaceRootPath: string, workspaceId: string): void {
    ...
    const callbacks: ConfigWatcherCallbacks = {
      ...
      onSkillChange: async (slug, skill) => {
        ...
        const { loadAllSkills } = await import('@sapstack-desktop/shared/skills')
        const skills = loadAllSkills(workspaceRootPath)
        this.broadcastSkillsChanged(workspaceId, skills)
      },
  ...
  private broadcastAppThemeChanged(theme: import('@sapstack-desktop/shared/config').ThemeOverrides | null): void {
    if (!this.eventSink) return
  ...
  private broadcastSkillsChanged(workspaceId: string, skills: import('@sapstack-desktop/shared/skills').LoadedSkill[]): void {
    if (!this.eventSink) return
  ...
  private async reloadSessionSources(managed: ManagedSession): Promise<void> {
    ...
    // Reload all sources from disk (sapstack-desktops-docs is always available as MCP server)
   ...
   * See: https://github.com/lukilabs/sapstack-desktops-oss/issues/39
   */
  /**
   * Reinitialize authentication environment variables.
   *
  ...
  async handleCredentialInput(
    sessionId: string,
    requestId: string,
    response: import('@sapstack-desktop/shared/protocol').CredentialResponse
  ): Promise<void> {
    const managed = this.sessions.get(sessionId)
    ...
    try {
      ...
      const { markSourceAuthenticated } = await import('@sapstack-desktop/shared/sources')
  ...
  async createSession(
    workspaceId: string,
    options?: import('@sapstack-desktop/shared/protocol').CreateSessionOptions,
    ...
    if (requestedProjectId) {
      const { loadProjectById } = await import('@sapstack-desktop/shared/projects')
    ...
    if (validatedBranch) {
      ...
      // full-history fork — see sapstack-desktops-oss#782.
      if (
  ...
  private async getOrCreateAgent(managed: ManagedSession): Promise<AgentInstance> {
    ...
    if (!managed.agent) {
      ...
      // The SDK subprocess reads SAPSTACK_DESKTOP_SESSION_DIR to write tool-metadata.json;
      ...
      process.env.SAPSTACK_DESKTOP_SESSION_DIR = sessionDirForMetadata
      ...
      const envOverrides: Record<string, string> = {
        SAPSTACK_DESKTOP_WORKSPACE_PATH: managed.workspace.rootPath,
      ...
      managed.agent = createBackendFromResolvedContext({
        ...
        enable1MContext: await (async () => { const { getEnable1MContext } = await import('@sapstack-desktop/shared/config/storage'); return getEnable1MContext(); })(),
      ...
      mergeSessionScopedToolCallbacks(managed.id, {
        ...
        // which must stay dependency-free of @sapstack-desktop/shared); the creation flow
        ...
        activateSourceInSessionFn: async (sourceSlug: string) => {
          ...
          // landing in a fresh turn with tools live (sapstack-desktops-oss#804).
  ...
  async setSessionConnection(sessionId: string, connectionSlug: string): Promise<void> {
    ...
    const { getLlmConnection } = await import('@sapstack-desktop/shared/config/storage')
   ...
   */
  async shareToViewer(sessionId: string): Promise<import('@sapstack-desktop/shared/protocol').ShareResult> {
    const managed = this.sessions.get(sessionId)
    if (!managed) {
    ...
    try {
      ...
      const { VIEWER_URL } = await import('@sapstack-desktop/shared/branding')
   ...
   */
  async updateShare(sessionId: string): Promise<import('@sapstack-desktop/shared/protocol').ShareResult> {
    const managed = this.sessions.get(sessionId)
    if (!managed) {
      return { success: false, error: 'Session not found' }
    }
   ...
   */
  async revokeShare(sessionId: string): Promise<import('@sapstack-desktop/shared/protocol').ShareResult> {
    const managed = this.sessions.get(sessionId)
    if (!managed) {
      return { success: false, error: 'Session not found' }
    }
  ...
  async deleteSession(sessionId: string): Promise<void> {
    ...
    // Cancel any pending source-activation auto-retry timer (sapstack-desktops-oss#804).
  ...
  async sendMessage(
    ...
    // Source-activation auto-retry dedup (sapstack-desktops-oss#804). When the server
    ...
    try {
      ...
      // in the rawText, and canUseTool in sapstack-desktop.ts provides a fallback
  ...
  respondToPermission(
    ...
    allowed: boolean,
    alwaysAllow: boolean,
    options?: import('@sapstack-desktop/shared/protocol').PermissionResponseOptions,
  ): boolean {
    const managed = this.sessions.get(sessionId)
    if (managed?.agent) {
   ...
   */
  async respondToCredential(sessionId: string, requestId: string, response: import('@sapstack-desktop/shared/protocol').CredentialResponse): Promise<boolean> {
  ...
  async executePromptAutomation(
    ...
    // client timeout (sapstack-desktops-oss#943). The session streams live either
  ...
  private async generateRemoteTransferSummary(managed: ManagedSession): Promise<string | null> {
    ...
    const envOverrides: Record<string, string> = {
      SAPSTACK_DESKTOP_WORKSPACE_PATH: workspaceRootPath,
      ...(miniModel ? { ANTHROPIC_DEFAULT_HAIKU_MODEL: miniModel } : {}),
    }
