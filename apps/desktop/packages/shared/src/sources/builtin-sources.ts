 ...
 *
 * NOTE: sapstack-desktops-docs is now an always-available MCP server configured
 * directly in sapstack-desktop.ts, not a source. This file is kept for backwards
 ...
 *
 * Currently returns empty array - sapstack-desktops-docs has been moved to
 * an always-available MCP server in sapstack-desktop.ts.
 *
...
/**
 * Get the built-in sapstack Desktop docs source.
 *
 * @deprecated sapstack-desktops-docs is now an always-available MCP server
 * configured directly in sapstack-desktop.ts. This function is kept for
...
export function getDocsSource(workspaceId: string, workspaceRootPath: string): LoadedSource {
  ...
  const placeholderConfig: FolderSourceConfig = {
    id: 'builtin-sapstack-desktops-docs',
    name: 'sapstack Desktop Docs',
    slug: 'sapstack-desktops-docs',
    enabled: false,
    provider: 'mintlify',
    type: 'mcp',
    mcp: {
      transport: 'http',
      url: 'https://boxlogodev.github.io/sapstack/docs/mcp',
      authType: 'none',
    },
    tagline: 'Search sapstack Desktop documentation and source setup guides',
    icon: '📚',
 ...
 *
 * Returns false - sapstack-desktops-docs is now an always-available MCP server,
