 ...
 * @param workspaceRootPath - Absolute path to workspace folder (e.g., ~/.sapstack-desktop/workspaces/xxx)
 ...
 * (like sapstack-desktops-docs) that don't have filesystem folders.
 */
export function getSourcesBySlugs(workspaceRootPath: string, slugs: string[]): LoadedSource[] {
  ...
  for (const slug of slugs) {
    ...
    if (isBuiltinSource(slug)) {
      // Currently only sapstack-desktops-docs is a builtin source
      if (slug === 'sapstack-desktops-docs') {
 ...
 * Built-in sources (like sapstack-desktops-docs) are always available and merged
...
export async function createSource(
  ...
  // (bundled guides removed - agent should search sapstack-desktops-docs MCP for service-specific guidance)
