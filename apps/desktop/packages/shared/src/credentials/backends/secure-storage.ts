 ...
 *
 * Stores credentials in an encrypted file at ~/.sapstack-desktop/credentials.enc
...
// File location
const CREDENTIALS_DIR = join(homedir(), '.sapstack-desktop');
...
export class SecureStorageBackend implements CredentialBackend {
  ...
  private getEncryptionKey(salt: Buffer): Buffer {
    ...
    const stableMachineId = createHash('sha256')
      ...
      .update('sapstack-desktop-v2') // Bumped version for new key derivation
      .digest();
