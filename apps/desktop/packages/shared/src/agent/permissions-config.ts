 ...
 *
 * File locations:
 * - Workspace: ~/.sapstack-desktop/workspaces/{slug}/permissions.json
 * - Per-source: ~/.sapstack-desktop/workspaces/{slug}/sources/{sourceSlug}/permissions.json
 *
 ...
 * Default permissions are stored at ~/.sapstack-desktop/permissions/
 * Reads env var dynamically so tests can override via SAPSTACK_DESKTOP_CONFIG_DIR.
 */
export function getAppPermissionsDir(): string {
  const configDir = process.env.SAPSTACK_DESKTOP_CONFIG_DIR || join(homedir(), '.sapstack-desktop');
...
/**
 * Load default permissions from ~/.sapstack-desktop/permissions/default.json
...
function shouldCompileBashPattern(pattern: string): boolean {
  if (!FEATURE_FLAGS.craftAgentsCli && pattern.startsWith('^sapstack-desktop\\s')) {
    return false;
  }
  return true;
}
...
class PermissionsConfigCache {
  ...
  // App-level default permissions (loaded from ~/.sapstack-desktop/permissions/default.json)
   ...
   * These come from ~/.sapstack-desktop/permissions/default.json
   */
  ...
  private applyDefaultConfig(merged: MergedPermissionsConfig, config: PermissionsCustomConfig): void {
    ...
    for (const patternEntry of config.allowedBashPatterns) {
      if (!shouldCompileBashPattern(patternEntry.pattern)) {
        debug(`[Permissions] Skipping sapstack-desktop bash pattern (feature disabled): ${patternEntry.pattern}`);
        continue;
      }
