 ...
 *
 * `SAPSTACK_DESKTOP_KEEP_BG_AGENTS_ALIVE`:
 ...
 * `SAPSTACK_DESKTOP_KEEP_BG_AGENTS_ALIVE=0` to fall back to the per-turn kill-switch.
 */
...
export function resolveKeepBackgroundTasksAlive(
  ...
  const raw = env.SAPSTACK_DESKTOP_KEEP_BG_AGENTS_ALIVE;
