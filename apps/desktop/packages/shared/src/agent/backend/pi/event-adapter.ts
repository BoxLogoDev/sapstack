 ...
 * sapstack Desktop's AgentEvent format for UI compatibility.
 *
...
import type { AgentEvent as CraftAgentEvent } from '@sapstack-desktop/core/types';
import type {
...
/**
 * Maps Pi SDK events to sapstack DesktopEvents for UI compatibility.
 *
 * Event mapping:
...
export class PiEventAdapter extends BaseEventAdapter {
  ...
  /**
   * Adapt a Pi SDK event to zero or more sapstack DesktopEvents.
   */
  *adaptEvent(event: PiEvent): Generator<CraftAgentEvent> {
    ...
    // it to a Craft assistant message via `sdkMessageId`. See sapstack-desktops-oss#782.
