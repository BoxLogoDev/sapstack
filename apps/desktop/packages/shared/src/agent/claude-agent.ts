...
// Plan types are used by UI components; not needed in sapstack-desktop.ts since Safe Mode is user-controlled
...
// Documentation is served via local files at ~/.sapstack-desktop/docs/
...
export class ClaudeAgent extends BaseAgent {
   ...
   * `SAPSTACK_DESKTOP_KEEP_BG_AGENTS_ALIVE` via the shared resolver. Default is ON (opt-out);
   * set `SAPSTACK_DESKTOP_KEEP_BG_AGENTS_ALIVE=0` to force the per-turn kill-switch path.
   */
  ...
  protected async *chatImpl(
    ...
    try {
      ...
      const fullMcpServers: Options['mcpServers'] = {
        ...
        // sapstack Desktop documentation - always available for searching setup guides
      ...
      // Gated by enable1MContext in global config (~/.sapstack-desktop/config.json).
          ...
          yield {
            ...
            error: isCwdCause ? {
              ...
              message:
                ...
                'or a security tool removed it). Reinstalling sapstack Desktop typically fixes this.',
              details: [
