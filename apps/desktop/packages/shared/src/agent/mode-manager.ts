...
export function getPathHint(targetPath: string, plansFolderPath: string, dataFolderPath?: string): string | null {
  ...
  if (normalizedTarget.includes('/.sapstack-desktop/workspaces/') && !normalizedTarget.includes('/sessions/')) {
    return 'Hint: Write to the session plans or data folder, not the workspace root.';
  }

  // Case: Writing outside .sapstack-desktop entirely
  if (!normalizedTarget.includes('/.sapstack-desktop/')) {
...
export function shouldAllowToolInMode(
  ...
  if (toolName.startsWith('mcp__')) {
    ...
    if (toolName.startsWith('mcp__sapstack-desktops-docs__')) {
      return { allowed: true };
    }
