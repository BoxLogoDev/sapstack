 ...
 * stderr was routed only to this.debug() which requires SAPSTACK_DESKTOP_DEBUG=1.
...
function createConfig(): BackendConfig {
  return {
    ...
    workspace: {
      id: 'ws-test',
      name: 'Test Workspace',
      rootPath: '/tmp/sapstack-desktop-test',
    } as any,
    session: {
      id: 'session-test',
      workspaceRootPath: '/tmp/sapstack-desktop-test',
      createdAt: Date.now(),
      lastUsedAt: Date.now(),
    } as any,
    isHeadless: true,
