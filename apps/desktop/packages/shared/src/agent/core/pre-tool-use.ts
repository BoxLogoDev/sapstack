...
import {
  CLI_DOMAIN_POLICIES,
  SAPSTACK_DESKTOP_AGENTS_CLI_BASH_GUARD_SCOPE_ENTRIES,
  SAPSTACK_DESKTOP_AGENTS_CLI_WORKSPACE_SCOPE_ENTRIES,
...
function buildCliDomainBlockMessage(namespace: CliDomainNamespace, context: string): string {
  ...
  return [
    `${context}`,
    `Use \`sapstack-desktop ${namespace} ...\` instead.`,
...
export function getConfigCliRedirect(
  ...
  if (filePath && LABELS_BLOCKED_FILE_TOOLS.has(toolName)) {
    ...
    if (relativePath) {
      const labelsScopeMatch = SAPSTACK_DESKTOP_AGENTS_CLI_WORKSPACE_SCOPE_ENTRIES.find(
...
/**
 * Block bash commands that operate on guarded config paths unless they use sapstack-desktop commands.
...
export function getConfigDomainBashRedirect(
  ...
  if (/^sapstack-desktop\s+(label|automation|source|skill)\b/.test(command)) {
    return null;
  }
  ...
  const bashGuardEntries: Array<{ namespace: CliDomainNamespace; scope: string }> = SAPSTACK_DESKTOP_AGENTS_CLI_BASH_GUARD_SCOPE_ENTRIES
...
const BUILT_IN_MCP_SERVERS = new Set(['session', 'sapstack-desktops-docs']);
...
export function runPreToolUseChecks(ctx: PreToolUseInput): PreToolUseCheckResult {
  ...
  // 5b. Config-domain Bash guard (block direct labels/automations path operations unless using sapstack-desktop)
