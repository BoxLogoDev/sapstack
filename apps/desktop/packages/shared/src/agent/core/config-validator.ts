...
/**
 * sapstack Desktop specific config files that have known schemas.
 */
const SAPSTACK_DESKTOP_AGENT_CONFIG_PATTERNS = [
  // Main config
  /\.sapstack-desktop\/config\.json$/,
  // Preferences
  /\.sapstack-desktop\/preferences\.json$/,
  // Source configs
  /\.sapstack-desktop\/workspaces\/[^/]+\/sources\/[^/]+\/config\.json$/,
  // Permissions
  /\.sapstack-desktop\/workspaces\/[^/]+\/permissions\.json$/,
  /\.sapstack-desktop\/permissions\/[^/]+\.json$/,
  // Theme
  /\.sapstack-desktop\/workspaces\/[^/]+\/theme\.json$/,
  // Statuses
  /\.sapstack-desktop\/workspaces\/[^/]+\/statuses\/config\.json$/,
  // Labels
  /\.sapstack-desktop\/workspaces\/[^/]+\/labels\.json$/,
  // Tool icons
  /\.sapstack-desktop\/tool-icons\/tool-icons\.json$/,
];
...
export class ConfigValidator {
  ...
  /**
   * Check if a file path is a sapstack Desktop config file.
   *
   * @param filePath - Path to check
   * @returns true if this is a sapstack Desktop config
   */
  isCraftAgentConfig(filePath: string): boolean {
    const normalizedPath = process.platform === 'win32'
      ? filePath.replace(/\\/g, '/').toLowerCase()
      : filePath.replace(/\\/g, '/');
    return SAPSTACK_DESKTOP_AGENT_CONFIG_PATTERNS.some((pattern) => pattern.test(normalizedPath));
  }
