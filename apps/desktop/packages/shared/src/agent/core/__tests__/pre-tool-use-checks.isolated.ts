...
describe('runPreToolUseChecks', () => {
  ...
  describe('step 2: source blocking', () => {
    ...
    it('skips source check for built-in MCP servers (sapstack-desktops-docs)', () => {
      const result = runPreToolUseChecks(createInput({
        toolName: 'mcp__sapstack-desktops-docs__search',
        input: {},
        activeSourceSlugs: [],
      }));
  ...
  describe('step 5: input transforms', () => {
    ...
    it('blocks direct label folder reads and suggests sapstack-desktop label help when feature is enabled', () => {
      mockCraftAgentsCliFlag = true;
      ...
      expect(result.type).toBe('block');
      if (result.type === 'block') {
        expect(result.reason).toContain('sapstack-desktop label');
        expect(result.reason).toContain('sapstack-desktop label --help');
        expect(result.reason).toContain('labels/');
      }
    });

    it('blocks direct label config writes and suggests sapstack-desktop label help when feature is enabled', () => {
      mockCraftAgentsCliFlag = true;
    ...
    it('blocks direct automations config edits and suggests sapstack-desktop automation commands when feature is enabled', () => {
      mockCraftAgentsCliFlag = true;
      ...
      expect(result.type).toBe('block');
      if (result.type === 'block') {
        expect(result.reason).toContain('sapstack-desktop automation');
        expect(result.reason).toContain('automations.json');
      }
    });

    it('blocks direct source config edits and suggests sapstack-desktop source commands when feature is enabled', () => {
      mockCraftAgentsCliFlag = true;
      ...
      expect(result.type).toBe('block');
      if (result.type === 'block') {
        expect(result.reason).toContain('sapstack-desktop source');
        expect(result.reason).toContain('sources/linear/config.json');
      }
    });

    it('blocks direct skill file edits and suggests sapstack-desktop skill commands when feature is enabled', () => {
      mockCraftAgentsCliFlag = true;
      mockDetectConfigFileType.mockImplementation(() => ({
        type: 'skill',
        slug: 'commit-helper',
      ...
      expect(result.type).toBe('block');
      if (result.type === 'block') {
        expect(result.reason).toContain('sapstack-desktop skill');
        expect(result.reason).toContain('skills/commit-helper/SKILL.md');
      }
    });

    it('blocks bash commands touching labels paths and points to sapstack-desktop label --help when feature is enabled', () => {
      mockCraftAgentsCliFlag = true;
    ...
    it('allows bash sapstack-desktop label commands through labels guard', () => {
      const result = runPreToolUseChecks(createInput({
        toolName: 'Bash',
        input: { command: 'sapstack-desktop label list' },
        permissionMode: 'allow-all',
      }));
    ...
    it('blocks bash commands touching automations files and points to sapstack-desktop automation --help when feature is enabled', () => {
      mockCraftAgentsCliFlag = true;
      ...
      expect(result.type).toBe('block');
      if (result.type === 'block') {
        expect(result.reason).toContain('sapstack-desktop automation --help');
        expect(result.reason).toContain('sapstack-desktop automation');
      }
    });

    it('allows bash sapstack-desktop automation commands through config-domain bash guard', () => {
      const result = runPreToolUseChecks(createInput({
        toolName: 'Bash',
        input: { command: 'sapstack-desktop automation list' },
        permissionMode: 'allow-all',
      }));
