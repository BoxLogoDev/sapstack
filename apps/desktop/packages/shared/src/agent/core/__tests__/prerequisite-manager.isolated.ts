...
function browserDocPath(): string {
  return resolve(join(homedir(), '.sapstack-desktop', 'docs', 'browser-tools.md'));
}
...
describe('PrerequisiteManager', () => {
  ...
  describe('rule matching', () => {
    ...
    it('exempts sapstack-desktops-docs MCP tools', () => {
      mockExistsPaths.add(guidePath('sapstack-desktops-docs'));
      const result = manager.checkPrerequisites('mcp__sapstack-desktops-docs__search');
      expect(result.allowed).toBe(true);
    });
