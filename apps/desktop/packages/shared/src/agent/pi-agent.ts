...
import type { AgentEvent } from '@sapstack-desktop/core/types';
...
// System prompt for sapstack Desktop context
  ...
  switch (code) {
    ...
    case 'CAPABILITY_UNAVAILABLE':
      return 'No connected desktop client supports browser tools, or no client is currently connected. ' +
        'Ask the user to open this workspace from the sapstack Desktop desktop app.';
...
export class PiAgent extends BaseAgent {
  protected backendName = 'sapstack Desktop Agent Runtime';
  ...
  // Ring buffer of recent subprocess stderr. Always on (independent of SAPSTACK_DESKTOP_DEBUG)
  ...
  private async spawnSubprocess(): Promise<void> {
    ...
    const child = spawn(nodePath, args, {
      ...
      env: {
        ...
        ...(sessionDir ? { SAPSTACK_DESKTOP_SESSION_DIR: sessionDir } : {}),
        // Propagate debug mode
        SAPSTACK_DESKTOP_DEBUG: (process.argv.includes('--debug') || process.env.SAPSTACK_DESKTOP_DEBUG === '1') ? '1' : '0',
      },
    });
    ...
    // Keep the SAPSTACK_DESKTOP_DEBUG-gated log for interactive dev work.
  ...
  private handleSubprocessEvent(event: Record<string, unknown>): void {
    ...
    // Feed them through PiEventAdapter to convert to sapstack DesktopEvents.
   ...
   * Uses the canonical registry from @sapstack-desktop/session-tools-core.
   */
  ...
  protected async *chatImpl(
    ...
    try {
      ...
      const systemPrompt = getSystemPrompt(
        ...
        'sapstack Desktop Agent Runtime', // backendName
