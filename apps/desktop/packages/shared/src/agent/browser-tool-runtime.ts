...
function getOpenVisibilitySettleTimeoutMs(override?: number): number {
  ...
  const envValue = Number(process.env.SAPSTACK_DESKTOP_BROWSER_OPEN_SETTLE_TIMEOUT_MS);
...
function getOpenVisibilitySettlePollMs(override?: number): number {
  ...
  const envValue = Number(process.env.SAPSTACK_DESKTOP_BROWSER_OPEN_SETTLE_POLL_MS);
  if (Number.isFinite(envValue) && envValue > 0) {
