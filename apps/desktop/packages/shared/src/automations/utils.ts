...
/**
 * Build the base SAPSTACK_DESKTOP_* environment variables shared by both prompt and webhook actions.
...
function buildBaseEventEnv(event: AutomationEvent, payload: BaseEventPayload): Record<string, string> {
  const env: Record<string, string> = {
    SAPSTACK_DESKTOP_EVENT: event,
    SAPSTACK_DESKTOP_EVENT_DATA: JSON.stringify(payload),
  };

  if (payload.sessionId) env.SAPSTACK_DESKTOP_SESSION_ID = payload.sessionId;
  if (payload.sessionName) env.SAPSTACK_DESKTOP_SESSION_NAME = payload.sessionName;
  if (payload.workspaceId) env.SAPSTACK_DESKTOP_WORKSPACE_ID = payload.workspaceId;
  ...
  if (Object.keys(sessionMetadata).length > 0) {
    env.SAPSTACK_DESKTOP_SESSION_METADATA = JSON.stringify(sessionMetadata);
  }

  // Local time for scheduler events
  if (event === 'SchedulerTick') {
    const now = new Date();
    env.SAPSTACK_DESKTOP_LOCAL_TIME = now.toLocaleTimeString('en-US', { hour12: false, hour: '2-digit', minute: '2-digit' });
    env.SAPSTACK_DESKTOP_LOCAL_DATE = now.toISOString().split('T')[0]!;
  }

  // Payload fields as SAPSTACK_DESKTOP_ vars (raw — callers apply sanitization if needed)
  for (const [key, value] of Object.entries(payload)) {
    if (PAYLOAD_SKIP_KEYS.has(key)) continue;
    const envKey = `SAPSTACK_DESKTOP_${toSnakeCase(key).toUpperCase()}`;
...
export function buildEnvFromPayload(event: AutomationEvent, payload: BaseEventPayload): Record<string, string> {
  ...
  if (payload.sessionName) env.SAPSTACK_DESKTOP_SESSION_NAME = sanitizeForShell(payload.sessionName);
 ...
 * - Only injects SAPSTACK_DESKTOP_WH_* user-defined vars from process.env (webhook secrets)
 * - Includes SAPSTACK_DESKTOP_* system vars derived from the event payload
 *
 ...
 *   export SAPSTACK_DESKTOP_WH_DISCORD_TOKEN="abc123"
 *
 * Then reference them in automations.json:
 *   "url": "${SAPSTACK_DESKTOP_WH_SLACK_URL}"
 *   "headers": { "Authorization": "Bearer ${SAPSTACK_DESKTOP_WH_DISCORD_TOKEN}" }
 */
export function buildWebhookEnv(event: AutomationEvent, payload: BaseEventPayload): Record<string, string> {
  ...
  // User-defined webhook secrets: only SAPSTACK_DESKTOP_WH_* from process.env
  for (const [key, value] of Object.entries(process.env)) {
    if (key.startsWith('SAPSTACK_DESKTOP_WH_') && value !== undefined) {
      env[key] = value;
    }
  }
