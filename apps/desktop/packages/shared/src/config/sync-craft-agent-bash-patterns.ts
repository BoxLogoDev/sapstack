...
function isCraftAgentPattern(entry: AllowedBashEntry): boolean {
  return typeof entry.pattern === 'string' && entry.pattern.startsWith('^sapstack-desktop\\s')
}
...
function main() {
  ...
  process.stdout.write(`Synced sapstack-desktop bash patterns in ${targetPath}\n`)
}
