...
import { SAPSTACK_DESKTOP_LOGO_HTML } from '../branding.ts';
...
<head>
  ...
  <title>sapstack Desktop - ${title}</title>
  <style>
...
<body>
  <div class="content">
    <pre class="logo">${SAPSTACK_DESKTOP_LOGO_HTML}</pre>
    <div class="card">
      <div class="status">${statusMessage}</div>
    </div>
    <div class="hint">${isSuccess ? 'You can now return to the application.' : 'Please close this window and try again.'}</div>
    ${deeplinkUrl ? `<a href="${deeplinkUrl}" class="return-link">sapstack Desktop</a>` : ''}
  </div>
