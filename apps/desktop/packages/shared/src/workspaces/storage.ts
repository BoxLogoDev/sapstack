 ...
 * Default location: ~/.sapstack-desktop/workspaces/
 */
...
import { seedSapstackKnowledge } from './sapstack-seed.ts';
...
const CONFIG_DIR = join(homedir(), '.sapstack-desktop');
...
/**
 * Get the default workspaces directory (~/.sapstack-desktop/workspaces/)
 */
...
export function loadWorkspace(rootPath: string): LoadedWorkspace | null {
  ...
  seedSapstackKnowledge(rootPath);
 ...
 * @param baseDir - Parent directory where workspace folders live (e.g., ~/.sapstack-desktop/workspaces/)
 ...
 * Returns paths to valid workspaces found in ~/.sapstack-desktop/workspaces/
 */
 ...
 * Resolution order: ENV (SAPSTACK_DESKTOP_LOCAL_MCP_ENABLED) > workspace config > default (true)
 *
 * @param rootPath - Absolute path to workspace root folder
...
export function isLocalMcpEnabled(rootPath: string): boolean {
  ...
  const envValue = process.env.SAPSTACK_DESKTOP_LOCAL_MCP_ENABLED;
...
export function ensurePluginManifest(rootPath: string, workspaceName: string): void {
  ...
  const manifest = {
    name: `sapstack-workspace-${workspaceName.toLowerCase().replace(/[^a-z0-9]+/g, '-')}`,
    version: '1.0.0',
  };
...
export { CONFIG_DIR, DEFAULT_WORKSPACES_DIR, seedSapstackKnowledge };
