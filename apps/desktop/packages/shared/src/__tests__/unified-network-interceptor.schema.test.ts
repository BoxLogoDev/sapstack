...
describe('unified-network-interceptor schema metadata injection', () => {
  beforeAll(async () => {
    process.env.SAPSTACK_DESKTOP_INTERCEPTOR_DISABLE_AUTO_INSTALL = '1';
...
describe('upgradePromptCacheTtl', () => {
  const configFile = join(homedir(), '.sapstack-desktop', 'config.json');
  ...
  function enableExtendedCache() {
    const dir = join(homedir(), '.sapstack-desktop');
