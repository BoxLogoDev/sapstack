# CLAUDE.md — `@sapstack-desktop/shared`

## Purpose
Core business logic package for sapstack Desktop:
...
- WebUI source OAuth uses a stable relay redirect URI (`https://boxlogodev.github.io/sapstack/auth/callback`); the deployment-specific callback target is carried in a relay-owned outer `state` envelope and unwrapped by the router worker.
...
- Automation matchers may declare an optional `telegramTopic?: string` to route spawned sessions into a Telegram forum topic in the workspace's paired supergroup. The field is plumbed through `PendingPrompt` and `ExecutePromptAutomationInput`; runtime resolution and topic creation live in `@sapstack-desktop/messaging-gateway`'s `TopicRegistry` and `MessagingGatewayRegistry.bindAutomationSession`. SessionManager picks up the resolution via the optional `setAutomationBinder` hook installed by the messaging-gateway bootstrap.
...
3. **Keep brand names in English**: Craft, sapstack Desktop, Agents, Workspace, Claude, Anthropic, OpenAI, MCP, API, SDK.
...
- **Main** hydrates on startup from `preferences.uiLanguage` in `~/.sapstack-desktop/preferences.json`. Maintained only by the `i18n:changeLanguage` IPC handler in `apps/electron/src/main/index.ts`.
