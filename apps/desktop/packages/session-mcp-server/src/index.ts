 ...
 * It uses the shared handlers from @sapstack-desktop/session-tools-core to ensure
 ...
 *   --workspace-root: Path to workspace folder (~/.sapstack-desktop/workspaces/{id})
...
import { isDeveloperFeedbackEnabled } from '@sapstack-desktop/shared/feature-flags';
...
import {
  ...
  errorResponse,
} from '@sapstack-desktop/session-tools-core';
...
function createCodexContext(config: SessionConfig): SessionToolContext {
  ...
  return {
    ...
    updatePreferences: (updates: Record<string, unknown>) => {
      ...
      // workspaceRootPath = ~/.sapstack-desktop/workspaces/{id}
      // preferencesPath = ~/.sapstack-desktop/preferences.json
    ...
    submitFeedback: (feedback) => {
      const configDir = process.env.SAPSTACK_DESKTOP_CONFIG_DIR || join(workspaceRootPath, '..', '..');
...
// ============================================================
// sapstack Desktop Docs Upstream Proxy
// ============================================================

const DOCS_MCP_URL = 'https://boxlogodev.github.io/sapstack/docs/mcp';
...
/**
 * Connect to the sapstack-desktops-docs MCP server and fetch its tool definitions.
...
async function connectDocsUpstream(): Promise<void> {
  try {
    const client = new Client(
      { name: 'sapstack-desktop-session-proxy', version: '1.0.0' },
      { capabilities: {} }
    );
    ...
    console.error(`sapstack Desktop Docs proxy connected: ${docsTools.length} tools`);
  } catch (err) {
    console.error(`sapstack Desktop Docs proxy connection failed (non-fatal): ${err instanceof Error ? err.message : String(err)}`);
    docsClient = null;
    docsTools = [];
  }
}
...
async function callDocsUpstream(
  ...
  if (!docsClient) {
    return errorResponse(`sapstack Desktop Docs server is not connected. Tool '${name}' unavailable.`);
  }
...
async function handleCallLlm(
  ...
  // Uses callbackPort from CLI arg (--callback-port) or env var (SAPSTACK_DESKTOP_LLM_CALLBACK_PORT).
  ...
  return errorResponse(
    ...
    'HTTP callback (SAPSTACK_DESKTOP_LLM_CALLBACK_PORT). Neither is available.'
  );
}
...
async function main() {
  ...
  const config: SessionConfig = {
    ...
    callbackPort: callbackPort || process.env.SAPSTACK_DESKTOP_LLM_CALLBACK_PORT,
  };
