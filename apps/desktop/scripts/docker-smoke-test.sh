#!/usr/bin/env bash
# =============================================================================
# Docker smoke test for sapstack Desktop Server
#
...
docker run -d \
  ...
  -e "SAPSTACK_DESKTOP_RPC_PORT=9100" \
  "$IMAGE"
...
while [ "$ELAPSED" -lt "$TIMEOUT" ]; do
  ...
  if docker logs "$CONTAINER_NAME" 2>&1 | grep -q "SAPSTACK_DESKTOP_SERVER_URL="; then
    READY=true
    break
  fi
