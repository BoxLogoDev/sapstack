# sapstack Desktop Windows Installer
# Usage: irm https://boxlogodev.github.io/sapstack/install-app.ps1 | iex

& {
$ErrorActionPreference = "Stop"

$VERSIONS_URL = "https://boxlogodev.github.io/sapstack/electron"
$DOWNLOAD_DIR = "$env:TEMP\sapstack-desktop-install"
$APP_NAME = "sapstack Desktop"
...
$process = Get-Process -Name "sapstack Desktop" -ErrorAction SilentlyContinue
if ($process) {
    Write-Info "Closing sapstack Desktop..."
...
Write-Info "Adding 'sapstack-desktops' command to PATH..."

$binDir = "$env:LOCALAPPDATA\sapstack Desktop\bin"
$cmdFile = "$binDir\sapstack-desktops.cmd"
$exePath = "$env:LOCALAPPDATA\Programs\sapstack Desktop\sapstack Desktop.exe"
...
if ($userPath -notlike "*$binDir*") {
    ...
    Write-Success "Added to PATH (restart terminal to use 'sapstack-desktops' command)"
} else {
    Write-Success "Command 'sapstack-desktops' is ready"
}
...
Write-Host ""
Write-Host "  sapstack Desktop has been installed."
Write-Host ""
Write-Host "  Launch from:"
Write-Host "    - Start Menu or desktop shortcut"
Write-Host "    - Command line: sapstack-desktops (restart terminal first)"
Write-Host ""
}
