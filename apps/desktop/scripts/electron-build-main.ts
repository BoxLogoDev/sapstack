...
async function buildPiAgentServer(): Promise<void> {
  ...
  const proc = spawn({
    cmd: [
      ...
      "--format", "esm",
      "--external", "koffi",
      // @smithy/core loads this CommonJS helper dynamically. Keep it as an
      // explicit production dependency and external so Electron packaging
      // includes it without Bun trying to resolve from a hoisted realpath.
      "--external", "@aws-crypto/crc32",
    ],
    cwd: ROOT_DIR,
    stdout: "inherit",
    stderr: "inherit",
...
async function buildWhatsAppWorker(): Promise<void> {
  ...
  const proc = spawn({
    cmd: [
      ...
      // call site and not used by sapstack Desktop (we send text + documents, no
