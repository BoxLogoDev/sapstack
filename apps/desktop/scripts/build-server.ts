#!/usr/bin/env bun
/**
 * Build script for standalone sapstack Desktop server.
 *
...
function showHelp(): void {
  console.log(`
Standalone server build script for sapstack Desktop
 ...
 * not node: builtins, not workspace @sapstack-desktop/* packages).
 */
    ...
    for (const entry of readdirSync(d, { withFileTypes: true })) {
      ...
      if (entry.isDirectory() && entry.name !== 'node_modules' && entry.name !== 'tests' && entry.name !== '__tests__') {
        ...
        while ((match = importRe.exec(content)) !== null) {
          ...
          if (spec.startsWith('.') || spec.startsWith('node:') || spec.startsWith('@sapstack-desktop/')) continue;
...
function copyWorkspacePackages(config: ServerBuildConfig): void {
  ...
  // SAPSTACK_DESKTOP_MESSAGING_WA_WORKER env resolution in packages/server/src/index.ts.
  const packages = [
    'server',
    'server-core',
    'shared',
...
function createRootConfig(config: ServerBuildConfig): void {
  ...
  // Root package.json with workspaces (Bun resolves @sapstack-desktop/* through this)
  const rootPkg = {
  ...
  const rootTsconfig = {
    compilerOptions: {
      ...
      paths: {
        '@sapstack-desktop/server-core/*': ['./packages/server-core/src/*'],
        '@sapstack-desktop/shared/*': ['./packages/shared/src/*'],
        '@sapstack-desktop/core/*': ['./packages/core/src/*'],
        '@sapstack-desktop/session-tools-core/*': ['./packages/session-tools-core/src/*'],
      },
    },
  };
  writeFileSync(join(outputDir, 'tsconfig.json'), JSON.stringify(rootTsconfig, null, 2) + '\n');

  // Create workspace symlinks in node_modules/@sapstack-desktop/
  // Bun needs these to resolve workspace package imports at runtime
  const scopeDir = join(outputDir, 'node_modules', '@sapstack-desktop');
  ...
  if (existsSync(packagesDir)) {
    for (const pkg of readdirSync(packagesDir)) {
      ...
      try {
        ...
        if (name.startsWith('@sapstack-desktop/')) {
          const shortName = name.replace('@sapstack-desktop/', '');
...
export SAPSTACK_DESKTOP_BUNDLED_ASSETS_ROOT="$ROOT"
export SAPSTACK_DESKTOP_IS_PACKAGED=true
export SAPSTACK_DESKTOP_APP_ROOT="$ROOT"
export SAPSTACK_DESKTOP_RESOURCES_PATH="$ROOT/resources"

# CLI tools (doc tools use uv + Python scripts)
export SAPSTACK_DESKTOP_UV="$ROOT/resources/bin/uv"
export SAPSTACK_DESKTOP_SCRIPTS="$ROOT/resources/scripts"
...
# sapstack Desktop Server — convenience entry point
...
echo "=== sapstack Desktop Server Setup ==="
echo ""
...
if [ -z "\${SAPSTACK_DESKTOP_SERVER_TOKEN:-}" ]; then
  TOKEN=\$(openssl rand -hex 32)
  cat > "$DIR/.env" <<ENVFILE
SAPSTACK_DESKTOP_SERVER_TOKEN=$TOKEN

# TLS — uncomment and set paths to enable wss://
# SAPSTACK_DESKTOP_RPC_TLS_CERT=/path/to/cert.pem
# SAPSTACK_DESKTOP_RPC_TLS_KEY=/path/to/key.pem
# SAPSTACK_DESKTOP_RPC_TLS_CA=/path/to/ca.pem
ENVFILE
  echo ""
  echo "Generated server token (saved to $DIR/.env)"
else
  TOKEN="\$SAPSTACK_DESKTOP_SERVER_TOKEN"
  echo ""
  echo "Using SAPSTACK_DESKTOP_SERVER_TOKEN from environment."
fi
...
if [ "\${1:-}" = "--systemd" ]; then
  ...
  SERVICE_USER="\${SAPSTACK_DESKTOP_USER:-\$(logname 2>/dev/null || echo craft)}"
  SERVICE_FILE="/etc/systemd/system/craft-server.service"

  cat > "$SERVICE_FILE" <<UNIT
[Unit]
Description=sapstack Desktop Server
...
Environment=SAPSTACK_DESKTOP_RPC_HOST=127.0.0.1
Environment=SAPSTACK_DESKTOP_RPC_PORT=9100
...
echo ""
echo "Quick start:"
echo "  SAPSTACK_DESKTOP_SERVER_TOKEN=$TOKEN $DIR/start.sh"
echo ""
...
ENV SAPSTACK_DESKTOP_IS_PACKAGED=true
ENV SAPSTACK_DESKTOP_BUNDLED_ASSETS_ROOT=/app
ENV SAPSTACK_DESKTOP_APP_ROOT=/app
ENV SAPSTACK_DESKTOP_RESOURCES_PATH=/app/resources
ENV SAPSTACK_DESKTOP_UV=/app/resources/bin/uv
ENV SAPSTACK_DESKTOP_SCRIPTS=/app/resources/scripts
ENV SAPSTACK_DESKTOP_RPC_HOST=0.0.0.0
ENV SAPSTACK_DESKTOP_RPC_PORT=9100
...
services:
  craft-server:
    ...
    ports:
      - "9100:9100"
    environment:
      - SAPSTACK_DESKTOP_SERVER_TOKEN=\${SAPSTACK_DESKTOP_SERVER_TOKEN:?Set SAPSTACK_DESKTOP_SERVER_TOKEN}
      - SAPSTACK_DESKTOP_RPC_PORT=9100
      # TLS — uncomment to enable wss://
      # - SAPSTACK_DESKTOP_RPC_TLS_CERT=/certs/cert.pem
      # - SAPSTACK_DESKTOP_RPC_TLS_KEY=/certs/key.pem
    volumes:
      - craft-data:/root/.sapstack-desktop
...
async function main(): Promise<void> {
  ...
  console.log(`=== Building sapstack Desktop Server ${version} for ${platform}-${arch} ===`);
  ...
  console.log(`\nQuick start:`);
  console.log(`  SAPSTACK_DESKTOP_SERVER_TOKEN=<secret> ${outputDir}/start.sh`);
}
