...
// Detects instance number from folder name suffix (e.g., sapstack-desktops-1 → instance 1)
function detectInstance(): void {
  // Don't override if already set (e.g., by sourcing detect-instance.sh first)
  if (process.env.SAPSTACK_DESKTOP_VITE_PORT) return;
  ...
  if (match) {
    ...
    process.env.SAPSTACK_DESKTOP_INSTANCE_NUMBER = instanceNum;
    process.env.SAPSTACK_DESKTOP_VITE_PORT = `${instanceNum}173`;
    process.env.SAPSTACK_DESKTOP_APP_NAME = `sapstack Desktop [${instanceNum}]`;
    process.env.SAPSTACK_DESKTOP_CONFIG_DIR = join(process.env.HOME || "", `.sapstack-desktop-${instanceNum}`);
    process.env.SAPSTACK_DESKTOP_DEEPLINK_SCHEME = `craftagents${instanceNum}`;
    console.log(`🔢 Instance ${instanceNum} detected: port=${process.env.SAPSTACK_DESKTOP_VITE_PORT}, config=${process.env.SAPSTACK_DESKTOP_CONFIG_DIR}`);
  }
}
...
function getElectronEnv(): Record<string, string> {
  const vitePort = process.env.SAPSTACK_DESKTOP_VITE_PORT || "5173";
  ...
  return {
    ...
    SAPSTACK_DESKTOP_CONFIG_DIR: process.env.SAPSTACK_DESKTOP_CONFIG_DIR || "",
    SAPSTACK_DESKTOP_APP_NAME: process.env.SAPSTACK_DESKTOP_APP_NAME || "sapstack Desktop",
    SAPSTACK_DESKTOP_DEEPLINK_SCHEME: process.env.SAPSTACK_DESKTOP_DEEPLINK_SCHEME || "craftagents",
    SAPSTACK_DESKTOP_INSTANCE_NUMBER: process.env.SAPSTACK_DESKTOP_INSTANCE_NUMBER || "",
  };
}
