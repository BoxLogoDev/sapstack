...
VERSIONS_URL="https://boxlogodev.github.io/sapstack/electron"
DOWNLOAD_DIR="$HOME/.sapstack-desktop/downloads"
...
if [ "$OS_TYPE" = "darwin" ]; then
    ...
    APP_BUNDLE_ID="com.lukilabs.sapstack-desktop"
    if pgrep -x "sapstack Desktop" >/dev/null 2>&1; then
        info "Quitting sapstack Desktop..."
        ...
        if pgrep -x "sapstack Desktop" >/dev/null 2>&1; then
            ...
            pkill -9 -x "sapstack Desktop" 2>/dev/null || true
    ...
    echo ""
    printf "%b\n" "  sapstack Desktop has been installed to ${BOLD}$INSTALL_DIR/$APP_NAME${NC}"
    echo ""
    printf "%b\n" "  You can launch it from ${BOLD}Applications${NC} or by running:"
    printf "%b\n" "    ${BOLD}open -a 'sapstack Desktop'${NC}"
    echo ""
...
#!/bin/bash
# sapstack Desktop launcher - handles Linux-specific AppImage issues

APPIMAGE_PATH="$HOME/.sapstack-desktop/app/Craft-Agents-x64.AppImage"
ELECTRON_CACHE="$HOME/.config/@sapstack-desktop"
ELECTRON_CACHE_ALT="$HOME/.cache/@sapstack-desktop"

# Verify AppImage exists
if [ ! -f "$APPIMAGE_PATH" ]; then
    echo "Error: sapstack Desktop not found at $APPIMAGE_PATH"
    echo "Reinstall: curl -fsSL https://boxlogodev.github.io/sapstack/install-app.sh | bash"
    exit 1
fi
...
WRAPPER_EOF
    ...
    echo ""
    printf "%b\n" "  Run with: ${BOLD}sapstack-desktops${NC}"
    echo ""
