#!/usr/bin/env bun
/**
 * craft-cli — Terminal client for sapstack Desktop server.
 *
 * Connects over WebSocket (ws:// or wss://) to a running sapstack Desktop server
...
export function parseArgs(argv: string[]): CliArgs {
  ...
  // Env var fallbacks
  if (!url) url = process.env.SAPSTACK_DESKTOP_SERVER_URL ?? ''
  if (!token) token = process.env.SAPSTACK_DESKTOP_SERVER_TOKEN ?? ''
  if (!tlsCa) tlsCa = process.env.SAPSTACK_DESKTOP_TLS_CA
...
function printHelp(): void {
  process.stdout.write(`craft-cli — Terminal client for sapstack Desktop server

Usage: craft-cli [options] <command> [args...]

Connection:
  --url <ws[s]://...>    Server URL (default: $SAPSTACK_DESKTOP_SERVER_URL)
  --token <secret>       Auth token (default: $SAPSTACK_DESKTOP_SERVER_TOKEN)
...
export async function main(argv: string[] = process.argv): Promise<void> {
  ...
  if (!args.url) {
    err('No server URL. Use --url <ws://...> or set $SAPSTACK_DESKTOP_SERVER_URL')
    process.exit(1)
  }
