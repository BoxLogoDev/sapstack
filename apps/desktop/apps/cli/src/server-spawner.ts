/**
 * Server spawner — start a headless sapstack Desktop server as a child process.
 *
 * Spawns `bun run <serverEntry>`, reads stdout for the `SAPSTACK_DESKTOP_SERVER_URL=`
 * and `SAPSTACK_DESKTOP_SERVER_TOKEN=` lines, and returns a handle to stop the server.
 */
...
export async function spawnServer(opts?: SpawnServerOptions): Promise<SpawnedServer> {
  ...
  const proc: Subprocess = Bun.spawn(['bun', 'run', serverEntry], {
    env: {
      ...
      SAPSTACK_DESKTOP_RPC_PORT: '0',
      SAPSTACK_DESKTOP_RPC_HOST: '127.0.0.1',
    },
    stdout: 'pipe',
    stderr: 'pipe',
  })
  ...
  // Read stdout line by line looking for SAPSTACK_DESKTOP_SERVER_URL=
  return new Promise<SpawnedServer>((resolve, reject) => {
    ...
    const processLines = () => {
      ...
      for (const line of lines) {
        if (line.startsWith('SAPSTACK_DESKTOP_SERVER_URL=')) {
          url = line.slice('SAPSTACK_DESKTOP_SERVER_URL='.length).trim()
        }
        if (line.startsWith('SAPSTACK_DESKTOP_SERVER_TOKEN=')) {
    ...
    ;(async () => {
      ...
      clearTimeout(timer)
      if (!url) {
        reject(new Error('Server process exited before printing SAPSTACK_DESKTOP_SERVER_URL'))
      }
    })()
  })
}
