...
import { TooltipProvider } from '@sapstack-desktop/ui'
...
import { extractWorkspaceSlugFromPath } from '@sapstack-desktop/shared/utils/workspace-slug'
import { DEFAULT_THINKING_LEVEL } from '@sapstack-desktop/shared/agent/thinking-levels'
...
export default function App() {
  ...
  const [sapEnvironment, setSapEnvironment] = useState<SapEnvironmentProfile | null | undefined>(undefined)
  const [sapEnvironmentError, setSapEnvironmentError] = useState<string>()

  useEffect(() => {
    window.sapstack.environment.get()
      .then(setSapEnvironment)
      .catch((error) => {
        setSapEnvironmentError(error instanceof Error ? error.message : 'SAP 환경 프로필을 읽지 못했습니다.')
  ...
  useEffect(() => {
    const initialize = async () => {
      try {
        ...
        if (needs.isFullyConfigured) {
          // If no workspace is selected (thin client without SAPSTACK_DESKTOP_WORKSPACE_ID),
  ...
  // Platform actions for @sapstack-desktop/ui components (overlays, etc.)
