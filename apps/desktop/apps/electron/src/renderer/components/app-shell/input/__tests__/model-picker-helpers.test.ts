...
import type { LlmConnection } from '@sapstack-desktop/shared/config/llm-connections'
import {
  formatTokenCount,
...
describe('groupConnectionsByProvider', () => {
  ...
  test('places "Anthropic" group before pi groups (display order)', () => {
    ...
    expect(result.map(([k]) => k)).toEqual(['Anthropic', 'sapstack Desktop Agent Runtime'])
  })
  ...
  test('"pi_compat" with remote baseUrl goes to "sapstack Desktop Agent Runtime"', () => {
    const remote = conn('openrouter', 'pi_compat', { baseUrl: 'https://openrouter.ai/api/v1' })
    const result = groupConnectionsByProvider([remote])
    expect(result).toEqual([['sapstack Desktop Agent Runtime', [remote]]])
  })

  test('drops empty groups from the output', () => {
    const a = conn('a', 'anthropic')
    const result = groupConnectionsByProvider([a])
    // Only "Anthropic" appears; "Local" and "sapstack Desktop Agent Runtime" are dropped.
  ...
  test('full mixed input — anthropic + local + remote pi_compat + pi', () => {
    ...
    expect(result.map(([k, conns]) => [k, conns.map(c => c.slug)])).toEqual([
      ['Anthropic', ['a']],
      ['Local', ['ollama']],
      ['sapstack Desktop Agent Runtime', ['or', 'p']],
    ])
  })
})
