 ...
 * Order is significant for UI: Anthropic, Local, sapstack Desktop Agent Runtime.
...
export function groupConnectionsByProvider<T extends LlmConnection>(
  ...
  const groups: Record<string, T[]> = {
    'Anthropic': [],
    'Local': [],
    'sapstack Desktop Agent Runtime': [],
  }
  for (const conn of connections) {
    ...
    if (provider === 'anthropic') {
      ...
      groups['sapstack Desktop Agent Runtime'].push(conn)
    }
  }
