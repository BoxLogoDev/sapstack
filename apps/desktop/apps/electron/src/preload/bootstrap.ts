 ...
 *
 * Thin-client mode (SAPSTACK_DESKTOP_SERVER_URL):
...
} from '@sapstack-desktop/server-core/transport'
import type { ConfirmDialogSpec, FileDialogSpec, BrowserCapabilityRequest } from '@sapstack-desktop/server-core/transport'
import type { RpcClient } from '@sapstack-desktop/server-core/transport'
import type { RemoteServerConfig } from '@sapstack-desktop/core/types'
...
if (isClientOnly) {
  ...
  const wsUrl = process.env.SAPSTACK_DESKTOP_SERVER_URL!
  const wsToken = process.env.SAPSTACK_DESKTOP_SERVER_TOKEN ?? ''
  ...
  if (parsed.protocol === 'ws:' && !isLocalhost) {
    throw new Error(
      ...
      `Set SAPSTACK_DESKTOP_RPC_TLS_CERT/KEY on the server to enable TLS.`
    )
  }

  // Workspace ID is optional — if missing, renderer shows a workspace picker
  const workspaceId = process.env.SAPSTACK_DESKTOP_WORKSPACE_ID || ipcRenderer.sendSync('__get-workspace-id') || undefined
...
contextBridge.exposeInMainWorld('sapstack', {
  catalog: {
    list: () => ipcRenderer.invoke('sapstack:catalog'),
  },
  knowledge: {
    resolveSymptom: (args: unknown) => ipcRenderer.invoke('sapstack:knowledge:resolveSymptom', args),
    checkTcode: (tcode: string) => ipcRenderer.invoke('sapstack:knowledge:checkTcode', tcode),
    resolveSapNote: (keyword: string) => ipcRenderer.invoke('sapstack:knowledge:resolveSapNote', keyword),
    lookupSynonym: (term: string) => ipcRenderer.invoke('sapstack:knowledge:lookupSynonym', term),
    getPrompt: (name: string, args?: Record<string, unknown>) => ipcRenderer.invoke('sapstack:knowledge:getPrompt', name, args || {}),
  },
  sessions: {
    start: (input: unknown) => ipcRenderer.invoke('sapstack:sessions:start', input),
    addEvidence: (input: unknown) => ipcRenderer.invoke('sapstack:sessions:addEvidence', input),
    submitHypotheses: (input: unknown) => ipcRenderer.invoke('sapstack:sessions:submitHypotheses', input),
    addFollowup: (input: unknown) => ipcRenderer.invoke('sapstack:sessions:addFollowup', input),
    submitVerdict: (input: unknown) => ipcRenderer.invoke('sapstack:sessions:submitVerdict', input),
    next: (input: unknown) => ipcRenderer.invoke('sapstack:sessions:next', input),
  ...
  connection: {
    get: () => ipcRenderer.invoke('sapstack:connection:get'),
