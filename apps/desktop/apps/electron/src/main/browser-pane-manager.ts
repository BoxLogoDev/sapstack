...
import { DEFAULT_THEME, loadAppTheme, getAllowRemoteEvaluate } from '@sapstack-desktop/shared/config'
import { CodedError } from '@sapstack-desktop/shared/protocol'
import { getBrowserLiveFxCornerRadii } from '../shared/browser-live-fx'
import type {
  IBrowserPaneManager,
  BrowserInstanceSnapshot,
} from '@sapstack-desktop/server-core/handlers'
import type {
...
const SAPSTACK_DESKTOP_DEEPLINK_SCHEME_PREFIX = `${process.env.SAPSTACK_DESKTOP_DEEPLINK_SCHEME || 'sapstack-desktop'}://`
...
export class BrowserPaneManager implements IBrowserPaneManager {
  ...
  private buildDeepLinkFromRoute(route: string): string {
    ...
    return `${SAPSTACK_DESKTOP_DEEPLINK_SCHEME_PREFIX}${normalizedPath}${routeQuery ? `?${routeQuery}` : ''}`
  }
  ...
  // validateUploadFilePath removed — uses shared validateFilePath from @sapstack-desktop/server-core/handlers
...
<html>
  ...
  private async handleDeepLinkUrl(url: string): Promise<void> {
    if (!url.startsWith(SAPSTACK_DESKTOP_DEEPLINK_SCHEME_PREFIX)) return
...
<html>
  ...
  private installThemeObserver(instance: BrowserInstance, allowRetry = true): void {
    ...
    void instance.pageView.webContents.executeJavaScript(`
      (() => {
        ...
        const w = window;
        const previousCleanup = w.__SAPSTACK_DESKTOP_THEME_OBSERVER_CLEANUP__;
        ...
        w.__SAPSTACK_DESKTOP_THEME_OBSERVER_CLEANUP__ = () => {
  ...
  private setupWindowListeners(instance: BrowserInstance): void {
    ...
    pageWc.on('will-navigate', (event, url) => {
      if (url.startsWith(SAPSTACK_DESKTOP_DEEPLINK_SCHEME_PREFIX)) {
        event.preventDefault()
    ...
    pageWc.setWindowOpenHandler((details) => {
      ...
      if (details.url.startsWith(SAPSTACK_DESKTOP_DEEPLINK_SCHEME_PREFIX)) {
