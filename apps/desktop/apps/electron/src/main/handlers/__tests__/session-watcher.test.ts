...
import type { RpcServer, RequestContext } from '@sapstack-desktop/server-core/transport'
...
describe('session file watcher isolation', () => {
  ...
  it('creates independent watchers per client and cleans up on disconnect', async () => {
    ...
    const { registerSessionsHandlers, cleanupSessionFileWatchForClient } = await import('@sapstack-desktop/server-core/handlers/rpc')
