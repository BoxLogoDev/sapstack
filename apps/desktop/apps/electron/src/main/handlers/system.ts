...
import { getGitBashPath, setGitBashPath, clearGitBashPath } from '@sapstack-desktop/shared/config'
import { classifyExternalUrl, formatBlockedUrlError } from '@sapstack-desktop/shared/utils/url-safety'
import { isUsableGitBashPath, validateGitBashPath } from '@sapstack-desktop/server-core/services'
...
export function registerSystemCoreHandlers(server: RpcServer, deps: HandlerDeps): void {
  ...
  server.handle(RPC_CHANNELS.releaseNotes.GET, async () => {
    const { getCombinedReleaseNotes } = require('@sapstack-desktop/shared/release-notes') as typeof import('@sapstack-desktop/shared/release-notes')
    return getCombinedReleaseNotes()
  })

  server.handle(RPC_CHANNELS.releaseNotes.GET_LATEST_VERSION, async () => {
    const { getLatestReleaseVersion } = require('@sapstack-desktop/shared/release-notes') as typeof import('@sapstack-desktop/shared/release-notes')
...
export function registerSystemGuiHandlers(server: RpcServer, deps: HandlerDeps): void {
  ...
  server.handle(RPC_CHANNELS.update.DISMISS, async (_ctx, version: string) => {
    const { setDismissedUpdateVersion } = await import('@sapstack-desktop/shared/config')
    setDismissedUpdateVersion(version)
  })

  server.handle(RPC_CHANNELS.update.GET_DISMISSED, async () => {
    const { getDismissedUpdateVersion } = await import('@sapstack-desktop/shared/config')
  ...
  server.handle(RPC_CHANNELS.notification.GET_ENABLED, async () => {
    const { getNotificationsEnabled } = await import('@sapstack-desktop/shared/config/storage')
    return getNotificationsEnabled()
  })

  server.handle(RPC_CHANNELS.notification.SET_ENABLED, async (_ctx, enabled: boolean) => {
    const { setNotificationsEnabled } = await import('@sapstack-desktop/shared/config/storage')
