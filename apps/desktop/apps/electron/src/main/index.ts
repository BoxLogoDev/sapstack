...
import { setupI18n, i18n, SUPPORTED_LANGUAGE_CODES, type LanguageCode } from '@sapstack-desktop/shared/i18n'
import { getPersistedUiLanguage, setPersistedUiLanguage } from '@sapstack-desktop/shared/config'
setupI18n()
...
import { RPC_CHANNELS } from '@sapstack-desktop/shared/protocol'
import { SessionManager, setSessionPlatform, setSessionRuntimeHooks } from '@sapstack-desktop/server-core/sessions'
import { registerAllRpcHandlers } from './handlers/index'
import { registerCoreRpcHandlers, cleanupSessionFileWatchForClient } from '@sapstack-desktop/server-core/handlers/rpc'
import type { PlatformServices } from '../runtime/platform'
import { createElectronPlatform } from './platform'
import type { HandlerDeps } from './handlers/handler-deps'
import { bootstrapServer, releaseServerLock } from '@sapstack-desktop/server-core/bootstrap'
import { createMessagingBootstrap, type MessagingBootstrapHandle } from '@sapstack-desktop/messaging-gateway'
import { getCredentialManager } from '@sapstack-desktop/shared/credentials'
import { initModelRefreshService, getModelRefreshService, setFetcherPlatform } from '@sapstack-desktop/server-core/model-fetchers'
import { setSearchPlatform, setImageProcessor } from '@sapstack-desktop/server-core/services'
...
import { getDefaultWorkspacesDir } from '@sapstack-desktop/shared/workspaces'
import { initializeDocs } from '@sapstack-desktop/shared/docs'
import { initializeReleaseNotes } from '@sapstack-desktop/shared/release-notes'
import { ensureDefaultPermissions } from '@sapstack-desktop/shared/agent/permissions-config'
import { ensureToolIcons, ensurePresetThemes } from '@sapstack-desktop/shared/config'
import { setBundledAssetsRoot } from '@sapstack-desktop/shared/utils'
import { initializeBackendHostRuntime } from '@sapstack-desktop/shared/agent/backend'
import { setPowerShellValidatorRoot } from '@sapstack-desktop/shared/agent'
import { handleDeepLink } from './deep-link'
import { BrowserPaneManager } from './browser-pane-manager'
import { OAuthFlowStore } from '@sapstack-desktop/shared/auth'
...
import { setPerfEnabled, enableDebug } from '@sapstack-desktop/shared/utils'
import { registerPiModelResolver } from '@sapstack-desktop/shared/config'
import { getPiModelsForAuthProvider, getAllPiModels } from '@sapstack-desktop/shared/config'
...
import { validateGitBashPath, checkVCRedistInstalled } from '@sapstack-desktop/server-core/services'
...
// These are available to all agent Bash sessions via SAPSTACK_DESKTOP_UV, SAPSTACK_DESKTOP_SCRIPTS env vars
// and PATH prepend. uv auto-downloads Python 3.12 on first use (~5s, then cached).
{
  ...
  process.env.SAPSTACK_DESKTOP_IS_PACKAGED = app.isPackaged ? '1' : '0'
  process.env.SAPSTACK_DESKTOP_RESOURCES_BASE = resourcesBase
  process.env.SAPSTACK_DESKTOP_APP_ROOT = app.isPackaged ? app.getAppPath() : process.cwd()

  process.env.SAPSTACK_DESKTOP_UV = bundledUvExists ? uvBinary : (fallbackUv ?? uvBinary)
  ...
  if (existsSync(bunBinary)) {
    process.env.SAPSTACK_DESKTOP_BUN = bunBinary
  }

  process.env.SAPSTACK_DESKTOP_SCRIPTS = scriptsDir
  process.env.SAPSTACK_DESKTOP_COMMANDS_ENTRY = app.isPackaged
    ? join(app.getAppPath(), 'packages', 'sapstack-desktops-commands', 'src', 'main.ts')
    : join(process.cwd(), 'packages', 'sapstack-desktops-commands', 'src', 'main.ts')
  process.env.SAPSTACK_DESKTOP_CLI_ENTRY = app.isPackaged
    ? join(app.getAppPath(), 'packages', 'craft-cli', 'src', 'cli.ts')
    : join(process.cwd(), 'packages', 'craft-cli', 'src', 'cli.ts')
  process.env.SAPSTACK_DESKTOP_COMMANDS_DOC_PATH = app.isPackaged
    ? join(resourcesBase, 'resources', 'docs', 'craft-cli.md')
    : join(process.cwd(), 'apps', 'electron', 'resources', 'docs', 'craft-cli.md')
  process.env.SAPSTACK_DESKTOP_CLI_DOC_PATH = process.env.SAPSTACK_DESKTOP_COMMANDS_DOC_PATH
  process.env.SAPSTACK_DESKTOP_AGENT_VERSION = app.getVersion()
  ...
  if (!bundledUvExists) {
    mainLog.warn('Bundled uv binary missing, CLI document tools may fail unless uv is available on PATH.', {
      ...
      usingCraftUv: process.env.SAPSTACK_DESKTOP_UV,
    })
  }

  if (isDebugMode) {
    mainLog.info('CLI tools configured:', { uvBinary: process.env.SAPSTACK_DESKTOP_UV, binDir, scriptsDir, bundledUvExists })
  }
}
...
// Supports multi-instance dev: SAPSTACK_DESKTOP_DEEPLINK_SCHEME env var (craftagents1, craftagents2, etc.)
const DEEPLINK_SCHEME = process.env.SAPSTACK_DESKTOP_DEEPLINK_SCHEME || 'sapstack-desktop'
...
// Supports multi-instance dev: SAPSTACK_DESKTOP_APP_NAME env var (e.g., "sapstack Desktop [1]")
app.setName(process.env.SAPSTACK_DESKTOP_APP_NAME || 'sapstack Desktop')
...
// Only bypasses cert validation for the exact SAPSTACK_DESKTOP_SERVER_URL origin — all other connections
...
if (process.env.SAPSTACK_DESKTOP_SERVER_URL) {
  let serverOrigin: string | undefined
  try {
    serverOrigin = normalizeOriginForCert(process.env.SAPSTACK_DESKTOP_SERVER_URL)
  } catch {
...
app.whenReady().then(async () => {
  ...
  process.env.SAPSTACK_DESKTOP_IS_PACKAGED = app.isPackaged ? 'true' : 'false'
  ...
  // Seed tool icons to ~/.sapstack-desktop/tool-icons/ (copies bundled SVGs on first run)
  ensureToolIcons()

  // Seed preset themes to ~/.sapstack-desktop/themes/ (copies bundled theme JSONs on first run)
  ensurePresetThemes()
  ...
  if (process.platform === 'darwin' && app.dock) {
    ...
    // SAPSTACK_DESKTOP_INSTANCE_NUMBER is set by detect-instance.sh for numbered folders
    const instanceNum = process.env.SAPSTACK_DESKTOP_INSTANCE_NUMBER
    if (instanceNum) {
  ...
  try {
    ...
    // When SAPSTACK_DESKTOP_SERVER_URL is set, this Electron instance is a thin client —
    ...
    const isClientOnly = !!process.env.SAPSTACK_DESKTOP_SERVER_URL
    const isHeadless = !!process.env.SAPSTACK_DESKTOP_HEADLESS

    if (isClientOnly) {
      mainLog.info(`Client-only mode: SAPSTACK_DESKTOP_SERVER_URL=${process.env.SAPSTACK_DESKTOP_SERVER_URL} (server initialization skipped)`)
    }
    ...
    if (!isClientOnly) {
      ...
      if (process.platform === 'win32') {
        const { getGitBashPath, clearGitBashPath } = await import('@sapstack-desktop/shared/config')
      ...
      if (process.platform === 'win32') {
        ...
        if (!vcCheck.installed) {
          ...
          process.env.SAPSTACK_DESKTOP_VCREDIST_MISSING = '1'
          if (vcCheck.downloadUrl) {
            process.env.SAPSTACK_DESKTOP_VCREDIST_URL = vcCheck.downloadUrl
          }
        } else if (isDebugMode) {
      ...
      const { getServerConfig } = await import('@sapstack-desktop/shared/config')
      ...
      const serverToken = serverModeEnabled && embeddedServerConfig.token
        ...
        : randomUUID()
      const rpcHost = process.env.SAPSTACK_DESKTOP_RPC_HOST
        ?? (serverModeEnabled ? '0.0.0.0' : '127.0.0.1')
      const rpcPort = process.env.SAPSTACK_DESKTOP_RPC_PORT
        ? parseInt(process.env.SAPSTACK_DESKTOP_RPC_PORT, 10)
        : (serverModeEnabled ? embeddedServerConfig.port : 0)

      // Load TLS certificates if configured
      let tls: import('@sapstack-desktop/server-core/transport').WsRpcTlsOptions | undefined
      ...
      const instance = await bootstrapServer<SessionManager, HandlerDeps>({
        ...
        createHandlerDeps: ({ sessionManager: sm, platform: p, oauthFlowStore: ofs }) => {
          ...
          messagingHandle = createMessagingBootstrap({
            ...
            getMessagingDir: (wsId: string) =>
              join(homedir(), '.sapstack-desktop', 'workspaces', wsId, 'messaging'),
            ...
            // at ~/.sapstack-desktop/logs/messaging-gateway.log.
        ...
        initModelRefreshService: () => initModelRefreshService(async (slug: string) => {
          const { getCredentialManager } = await import('@sapstack-desktop/shared/credentials')
      ...
      ipcMain.handle('workspace:remove', async (_event, workspaceId: string) => {
        const { removeWorkspace: remove } = await import('@sapstack-desktop/shared/config')
      ...
      ipcMain.handle('session:transferToWorkspace', async (_event, sessionId: string, targetWorkspaceId: string, sessionIndex?: number, sessionCount?: number) => {
        ...
        const { getWorkspaceByNameOrId } = await import('@sapstack-desktop/shared/config')
        const { connectToRemote } = await import('./handlers/workspace')
      ...
      instance.wsServer.handle(RPC_CHANNELS.settings.GET_SERVER_CONFIG, async () => {
        const { getServerConfig: getConfig } = await import('@sapstack-desktop/shared/config')
        return getConfig()
      })

      instance.wsServer.handle(RPC_CHANNELS.settings.SET_SERVER_CONFIG, async (_ctx: unknown, config: unknown) => {
        const { setServerConfig: setConfig } = await import('@sapstack-desktop/shared/config')
        const cfg = config as import('@sapstack-desktop/shared/config/server-config').ServerConfig
        // Validate port range
      ...
      if (isHeadless) {
        console.log(`SAPSTACK_DESKTOP_SERVER_URL=${instance.protocol}://${instance.host}:${instance.port}`)
        console.log(`SAPSTACK_DESKTOP_SERVER_TOKEN=${instance.token}`)
      }
    }
    ...
    try {
      const { getLlmConnection, getDefaultLlmConnection } = await import('@sapstack-desktop/shared/config')
...
app.on('window-all-closed', () => {
  if (process.env.SAPSTACK_DESKTOP_HEADLESS) return  // headless server stays alive
