...
export function parseDeepLink(url: string): DeepLinkTarget | null {
  try {
    ...
    // Keep the upstream scheme readable so existing links continue to work,
    // while all newly generated and registered links use sapstack Desktop.
    if (!['sapstack-desktop:', 'craftagents:'].includes(parsed.protocol)) {
      return null
    }
