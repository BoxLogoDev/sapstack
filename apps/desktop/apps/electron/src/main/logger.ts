...
import type {
  ...
  MessagingLogMeta,
  MessagingLogger,
} from '@sapstack-desktop/messaging-gateway'
 ...
 * 2) SAPSTACK_DESKTOP_IS_PACKAGED env (when explicitly set)
...
function resolveDebugMode(): boolean {
  ...
  const packagedEnv = process.env.SAPSTACK_DESKTOP_IS_PACKAGED
 ...
 */
export const messagingGatewayLogPath = join(homedir(), '.sapstack-desktop', 'logs', 'messaging-gateway.log')
 ...
 */
export const autoUpdateLogPath = join(homedir(), '.sapstack-desktop', 'logs', 'auto-update.log')
