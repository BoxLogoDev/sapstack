...
> **CLI-first workflow (recommended):** Use `sapstack-desktop label ...` commands instead of editing JSON directly.
> - `sapstack-desktop label --help`
> - Canonical command reference: [craft-cli.md](./craft-cli.md)

## Storage Locations

- Config: `~/.sapstack-desktop/workspaces/{id}/labels/config.json`
...
Prefer `sapstack-desktop` commands:

```bash
sapstack-desktop label create --name "Bug" --color "destructive"
sapstack-desktop label create --name "Priority" --color "accent" --value-type number
sapstack-desktop label create --name "Due Date" --color "info" --value-type date
sapstack-desktop label create --name "Docs" --color "info" --value-type link
sapstack-desktop label create --name "Project" --color "foreground/60"
sapstack-desktop label create --name "Alpha" --color "info" --parent-id project
sapstack-desktop label create --name "Beta" --color "success" --parent-id project
```
