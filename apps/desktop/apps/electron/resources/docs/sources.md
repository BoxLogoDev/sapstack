...
This guide explains how to configure sources (MCP servers, APIs, local filesystems) in sapstack Desktop.

> **CLI-first workflow (recommended):** Use `sapstack-desktop source ...` commands instead of editing source config files directly.
...
**Before doing anything else**, search for a specialized guide using the sapstack-desktops-docs MCP:

```
mcp__sapstack-desktops-docs__SearchCraftAgents({ query: "{service} source setup" })
```
...
Agent: [FIRST: Searches for Linear guide]
       mcp__sapstack-desktops-docs__SearchCraftAgents({ query: "linear source setup" })
...
- `~/.sapstack-desktop/workspaces/{workspaceId}/sources/{sourceSlug}/`
...
`~/.sapstack-desktop/provider-domains.json`
...
1. Create the source folder:
   ```bash
   mkdir -p ~/.sapstack-desktop/workspaces/{ws}/sources/my-source
   ```
