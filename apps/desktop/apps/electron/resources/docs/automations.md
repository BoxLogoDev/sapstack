...
This guide explains how to configure automations in sapstack Desktop to automate workflows based on events.

> **CLI-first workflow (recommended):** Use `sapstack-desktop automation ...` commands instead of editing JSON directly.
> - `sapstack-desktop automation --help`
> - Canonical command reference: [craft-cli.md](./craft-cli.md)

## What Are Automations?

Automations allow you to trigger actions automatically when specific events occur in sapstack Desktop. You can:
...
```
~/.sapstack-desktop/workspaces/{workspaceId}/automations.json
```
...
sapstack-desktop automation create --event UserPromptSubmit --prompt "..."
sapstack-desktop automation update <id> --json '{...}'
...
### App Events (triggered by sapstack Desktop)
...
Send a prompt to sapstack Desktop (creates a new session for scheduled prompts).
...
- Environment variables are expanded (e.g., `$SAPSTACK_DESKTOP_LABEL`)
...
{
  "type": "webhook",
  "url": "https://hooks.slack.com/services/${SAPSTACK_DESKTOP_WH_SLACK_PATH}",
  "method": "POST",
  "body": {
    "text": "Session ${SAPSTACK_DESKTOP_SESSION_NAME} status changed to ${SAPSTACK_DESKTOP_NEW_STATE}"
  }
}
```
...
```json
{
  ...
  "auth": {
    "type": "bearer",
    "token": "${SAPSTACK_DESKTOP_WH_API_TOKEN}"
  },
  "body": { "event": "$SAPSTACK_DESKTOP_EVENT" }
}
```
...
```json
{
  ...
  "auth": {
    "type": "basic",
    "username": "${SAPSTACK_DESKTOP_WH_USER}",
    "password": "${SAPSTACK_DESKTOP_WH_PASS}"
  }
}
```
...
**Security:** Webhook actions only have access to `SAPSTACK_DESKTOP_*` system variables and `SAPSTACK_DESKTOP_WH_*` user-defined secrets. They do **not** have access to your full system environment (e.g., `$HOME`, `$PATH`, or other process variables).

## Environment Variables

Both prompt and webhook actions support variable expansion using `$VAR` or `${VAR}` syntax.

### System Variables (SAPSTACK_DESKTOP_*)

These are automatically set by the automation system based on the triggering event:

| Variable | Description | Available For |
|----------|-------------|---------------|
| `$SAPSTACK_DESKTOP_EVENT` | Event name (e.g., `LabelAdd`) | All events |
| `$SAPSTACK_DESKTOP_EVENT_DATA` | Full event payload as JSON | All events |
| `$SAPSTACK_DESKTOP_SESSION_ID` | Session ID | Events with session context |
| `$SAPSTACK_DESKTOP_SESSION_NAME` | Session name | Events with session context |
| `$SAPSTACK_DESKTOP_WORKSPACE_ID` | Workspace ID | All events |

**Per-event variables:**

| Event | Variable | Description |
|-------|----------|-------------|
| `LabelAdd` / `LabelRemove` | `$SAPSTACK_DESKTOP_LABEL` | The label that was added/removed |
| `PermissionModeChange` | `$SAPSTACK_DESKTOP_OLD_MODE`, `$SAPSTACK_DESKTOP_NEW_MODE` | Previous and new permission mode |
| `FlagChange` | `$SAPSTACK_DESKTOP_IS_FLAGGED` | `true` or `false` |
| `SessionStatusChange` | `$SAPSTACK_DESKTOP_OLD_STATE`, `$SAPSTACK_DESKTOP_NEW_STATE` | Previous and new status |
| `SchedulerTick` | `$SAPSTACK_DESKTOP_LOCAL_TIME`, `$SAPSTACK_DESKTOP_LOCAL_DATE` | Current time (`14:30`) and date (`2026-03-09`) |

### User-Defined Webhook Secrets (SAPSTACK_DESKTOP_WH_*)

For webhook actions, you can define your own secrets by setting environment variables with the `SAPSTACK_DESKTOP_WH_` prefix in your shell profile (e.g., `~/.zshrc`, `~/.bashrc`):

```bash
# In your shell profile
export SAPSTACK_DESKTOP_WH_SLACK_URL="https://hooks.slack.com/services/T.../B.../xxx"
export SAPSTACK_DESKTOP_WH_DISCORD_URL="https://discord.com/api/webhooks/123/abc"
export SAPSTACK_DESKTOP_WH_API_TOKEN="your-secret-token"
```
...
{
  "type": "webhook",
  "url": "${SAPSTACK_DESKTOP_WH_SLACK_URL}",
  "method": "POST",
  "body": { "text": "Hello from sapstack Desktop!" }
}
```
...
```json
{
  "type": "webhook",
  "url": "https://api.example.com/events",
  "headers": { "Authorization": "Bearer ${SAPSTACK_DESKTOP_WH_API_TOKEN}" },
  "body": { "event": "${SAPSTACK_DESKTOP_EVENT}", "session": "${SAPSTACK_DESKTOP_SESSION_NAME}" }
}
```

This keeps secrets out of `automations.json` (which may be shared or committed to version control).

> **Note:** Only variables prefixed with `SAPSTACK_DESKTOP_WH_` are injected into webhook actions. Other environment variables (like `$HOME` or `$DATABASE_URL`) are not accessible to webhooks.
...
4. **Pair the supergroup.** In sapstack Desktop: Settings → Messaging → Telegram → **Pair Supergroup**. Copy the 6-digit code, then in any topic of the supergroup type `/pair <code>`. The bot confirms and the Settings row updates with the group's title.

Verify by checking the supergroup row in Settings shows the group title. If automation runs fail later, `~/.sapstack-desktop/logs/messaging-gateway.log` will show `automation_topic_bind_failed` with the underlying Telegram error.
        ...
        "actions": [
          {
            ...
            "url": "${SAPSTACK_DESKTOP_WH_SLACK_URL}",
            "method": "POST",
            "body": { "text": ":warning: Permission escalated from safe to allow-all in *${SAPSTACK_DESKTOP_SESSION_NAME}*" }
          }
        ]
      }
    ]
...
```json
{
  ...
  "automations": {
    "LabelAdd": [
      {
        "actions": [
          { "type": "prompt", "prompt": "The label $SAPSTACK_DESKTOP_LABEL was added. Log this change with a timestamp." }
        ]
      }
    ],
    "LabelRemove": [
      {
        "actions": [
          { "type": "prompt", "prompt": "The label $SAPSTACK_DESKTOP_LABEL was removed. Log this change with a timestamp." }
        ]
      }
    ]
  }
...
Sends a Slack message when a session is marked as done. Requires `SAPSTACK_DESKTOP_WH_SLACK_URL` in your shell profile.
        ...
        "actions": [
          {
            ...
            "method": "POST",
            "body": {
              "text": ":white_check_mark: Session *${SAPSTACK_DESKTOP_SESSION_NAME}* marked as done"
            }
          }
        ]
      }
        ...
        "actions": [
          {
            ...
            "url": "${SAPSTACK_DESKTOP_WH_SLACK_URL}",
            "method": "POST",
            "body": { "text": ":rotating_light: Urgent label added to *${SAPSTACK_DESKTOP_SESSION_NAME}*" }
          },
          {
            "type": "prompt",
        ...
        "actions": [
          {
            ...
            "body": {
              ...
              "client_id": "${SAPSTACK_DESKTOP_WH_CLIENT_ID}",
              "client_secret": "${SAPSTACK_DESKTOP_WH_CLIENT_SECRET}"
            }
          }
        ]
      }
        ...
        "actions": [
          {
            ...
            "method": "POST",
            "headers": {
              "Authorization": "Bearer ${SAPSTACK_DESKTOP_WH_API_TOKEN}",
              "X-Source": "sapstack-desktop"
            },
            "body": {
              "event": "${SAPSTACK_DESKTOP_EVENT}",
              "session_id": "${SAPSTACK_DESKTOP_SESSION_ID}",
              "old_status": "${SAPSTACK_DESKTOP_OLD_STATE}",
              "new_status": "${SAPSTACK_DESKTOP_NEW_STATE}"
            }
          }
        ]
      }
...
Ask sapstack Desktop to validate your automations configuration:
...
2. **Check env vars** — Ensure `SAPSTACK_DESKTOP_WH_*` variables are set in your shell profile and sapstack Desktop was restarted after adding them. URLs using `$VAR` templates are validated after variable expansion — if the variable is empty or unset, the URL will be invalid.
...
5. **Keep secrets out of config** - Use `SAPSTACK_DESKTOP_WH_*` env vars for webhook URLs and tokens instead of hardcoding them in automations.json
