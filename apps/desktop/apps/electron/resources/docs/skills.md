...
This guide explains how to create and configure skills in sapstack Desktop.

> **CLI-first workflow (recommended):** Use `sapstack-desktop skill ...` commands instead of editing `SKILL.md` files directly.
...
sapstack Desktop uses **the identical SKILL.md format** as the Claude Code SDK. This means:

1. **Format compatibility**: Any skill written for Claude Code works in sapstack Desktop
...
1. **Workspace skill checked first** - If `~/.sapstack-desktop/workspaces/{id}/skills/commit/SKILL.md` exists, it's used
...
```
~/.sapstack-desktop/workspaces/{workspaceId}/skills/{slug}/
...
```bash
mkdir -p ~/.sapstack-desktop/workspaces/{ws}/skills/my-skill
```
...
1. Create `~/.sapstack-desktop/workspaces/{ws}/skills/commit/SKILL.md`
