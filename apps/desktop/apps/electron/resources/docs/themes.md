...
This guide explains how to customize the visual theme of sapstack Desktop.

## Overview

sapstack Desktop uses a 6-color theme system with support for both app-level defaults and per-workspace overrides.
...
3. **Preset themes**: `~/.sapstack-desktop/themes/{name}.json` - Complete theme packages
4. **Theme overrides**: `~/.sapstack-desktop/theme.json` - Override specific colors (app-level)
...
```
~/.sapstack-desktop/workspaces/{id}/config.json
```
...
Create `~/.sapstack-desktop/theme.json` to override specific colors:
...
Preset themes are complete theme packages stored at `~/.sapstack-desktop/themes/`. Each preset is a JSON file with theme colors and metadata.
...
2. Save it to `~/.sapstack-desktop/themes/{name}.json`
...
1. Create `~/.sapstack-desktop/theme.json` for overrides or `~/.sapstack-desktop/themes/{name}.json` for a preset
...
- Check file is in correct location (`~/.sapstack-desktop/theme.json` for overrides, `~/.sapstack-desktop/themes/` for presets)
