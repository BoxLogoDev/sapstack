...
## How it Works (Video)
To understand what sapstack Desktop does and how it works watch this video.
...
## Why sapstack Desktop was built
sapstack Desktop is a tool we built so that we (at craft.do) can work effectively with agents. It enables intuitive multitasking, no-fluff connection to any API or Service, sharing sessions, and a more document (vs code) centric workflow - in a beautiful and fluid UI.
...
sapstack Desktop is open source under the Apache 2.0 license - so you are free to remix, change anything. And that's actually possible. We ourselves are building sapstack Desktop with sapstack Desktop only - no code editors - so really, any customisation is just a prompt away.

We built sapstack Desktop because we wanted a better, more opinionated (and preferably non-CLI way) of working with the most powerful agents in the world. We'll continue to improve it, based on our experiences and intuition.
...
[Check out how I just connected to Slack →](https://boxlogodev.github.io/sapstack/s/DRNQEiy8w2e1v5LPgKl8b)
...
sapstack Desktop connects to anything. We have it hooked up to a direct Postgres DB behind a jumpbox. Skills + Sources = magic.
...
[Here I imported all my skills in one go →](https://boxlogodev.github.io/sapstack/s/gWCFqwhObFWaNJIEJmd6j)
...
**macOS / Linux:**
```bash
curl -fsSL https://boxlogodev.github.io/sapstack/install-app.sh | bash
```

**Windows (PowerShell):**
```powershell
irm https://boxlogodev.github.io/sapstack/install-app.ps1 | iex
```
...
sapstack Desktop can run as a headless server on a remote machine (e.g., a Linux VPS), with the desktop app connecting as a thin client. This lets you keep long-running sessions alive, access them from multiple machines, and run compute-heavy tasks on a powerful server.
...
SAPSTACK_DESKTOP_SERVER_TOKEN=$(openssl rand -hex 32) bun run packages/server/src/index.ts
```

The server prints the connection details on startup:

```
SAPSTACK_DESKTOP_SERVER_URL=ws://203.0.113.5:9100
SAPSTACK_DESKTOP_SERVER_TOKEN=<generated-token>
```
...
```bash
SAPSTACK_DESKTOP_SERVER_URL=wss://203.0.113.5:9100 SAPSTACK_DESKTOP_SERVER_TOKEN=<token> bun run electron:start
```
...
|----------|----------|---------|-------------|
| `SAPSTACK_DESKTOP_SERVER_TOKEN` | Yes | — | Bearer token for client authentication |
| `SAPSTACK_DESKTOP_RPC_HOST` | No | `127.0.0.1` | Bind address (`0.0.0.0` for remote access) |
| `SAPSTACK_DESKTOP_RPC_PORT` | No | `9100` | Bind port |
| `SAPSTACK_DESKTOP_RPC_TLS_CERT` | No | — | Path to PEM certificate file (enables `wss://`) |
| `SAPSTACK_DESKTOP_RPC_TLS_KEY` | No | — | Path to PEM private key file (required with cert) |
| `SAPSTACK_DESKTOP_RPC_TLS_CA` | No | — | Path to PEM CA chain file (optional, for client cert verification) |
| `SAPSTACK_DESKTOP_DEBUG` | No | `false` | Enable debug logging |
...
```bash
SAPSTACK_DESKTOP_SERVER_TOKEN=<token> \
SAPSTACK_DESKTOP_RPC_HOST=0.0.0.0 \
SAPSTACK_DESKTOP_RPC_TLS_CERT=certs/cert.pem \
SAPSTACK_DESKTOP_RPC_TLS_KEY=certs/key.pem \
bun run packages/server/src/index.ts
```

The server will print `SAPSTACK_DESKTOP_SERVER_URL=wss://<your-public-ip>:9100`.
...
docker run -d \
  -p 9100:9100 \
  -e SAPSTACK_DESKTOP_SERVER_TOKEN=<token> \
  -e SAPSTACK_DESKTOP_RPC_HOST=0.0.0.0 \
  -v craft-data:/root/.sapstack-desktop \
  sapstack-desktops-server
```
...
docker run -d \
  ...
  -e SAPSTACK_DESKTOP_SERVER_TOKEN=<token> \
  -e SAPSTACK_DESKTOP_RPC_HOST=0.0.0.0 \
  -e SAPSTACK_DESKTOP_RPC_TLS_CERT=/certs/cert.pem \
  -e SAPSTACK_DESKTOP_RPC_TLS_KEY=/certs/key.pem \
  -v ./certs:/certs:ro \
  -v craft-data:/root/.sapstack-desktop \
  sapstack-desktops-server
```
...
A terminal client that connects to a running sapstack Desktop server over WebSocket (`ws://` or `wss://`). Use it for scripting, CI/CD pipelines, server validation, or when you prefer the command line.
...
export SAPSTACK_DESKTOP_SERVER_URL=ws://127.0.0.1:9100
export SAPSTACK_DESKTOP_SERVER_TOKEN=<your-token>
...
# Debug logging (writes to ~/Library/Logs/@sapstack-desktop/electron/)
...
3. Fill in required fields:
   - App name: e.g., "My sapstack Desktop"
...
4. Name: e.g., "sapstack Desktop Desktop"
5. Click **Create**
6. Note the **Client ID** and **Client Secret**

#### 5. Configure in sapstack Desktop
...
sapstack Desktop supports multiple ways to connect to LLM providers:
...
sapstack Desktop uses two agent backends:
...
Configuration is stored at `~/.sapstack-desktop/`:
...
Or configure manually in `~/.sapstack-desktop/workspaces/{id}/automations.json`:
...
**Prompt actions** create a new agent session with a prompt. They support `@mentions` for sources and skills, and environment variables like `$SAPSTACK_DESKTOP_LABEL` and `$SAPSTACK_DESKTOP_SESSION_ID` are expanded automatically.
...
See the [Automations documentation](https://boxlogodev.github.io/sapstack/docs/automations/overview) for the full reference.
...
**Windows (PowerShell):**
```powershell
& "$env:LOCALAPPDATA\Programs\@sapstack-desktopelectron\sapstack Desktop.exe" -- --debug
```

**Linux:**
```bash
./sapstack-desktops -- --debug
```

Logs are written to:
- **macOS:** `~/Library/Logs/@sapstack-desktop/electron/main.log`
- **Windows:** `%APPDATA%\@sapstack-desktop\electron\logs\main.log`
- **Linux:** `~/.config/@sapstack-desktop/electron/logs/main.log`
...
"Craft" and "sapstack Desktop" are trademarks of Craft Docs Ltd. See [TRADEMARK.md](TRADEMARK.md) for usage guidelines.
