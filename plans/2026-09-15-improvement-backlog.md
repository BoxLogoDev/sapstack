...
| 2   | ✅ CBO 야간 스케줄러 미실행             | `register-task.ps1`: 배터리 시작 허용·StartWhenAvailable·robocopy `/R:2 /W:5`. 09-15 12:25 사내망 복귀 후 `Start-ScheduledTask`로 실제 수집 기동 — 열거 124,363건, 약 400건/분 → 17:30 전후 완료 예상(직전 08-30 스냅샷은 34,460건). 12:03 기동분은 네트워크 불통으로 0건 가드 작동 | 완료 후 `manifest.yaml`의 `exported_at`·`status`·`totals` 확인. 게시 robocopy는 DNS(#5)로 실패 예상 — 스냅샷은 로컬에 남는다 | 🤖   | manifest `exported_at` 09-15T03Z 이후 + status complete                   |
...
| 6   | 배포 전 시크릿 25건 검토           | ✅ 열람(09-15): 25건 중 24건은 주석 처리된 `PASSWORD = ' '` 자리표시자(FTP/RFC 호출 블록) — 오탐. `scrub.mjs` 규칙에서 공백 리터럴 제외 + 테스트 추가 → 재스캔 1건. 남은 1건 `zhr0/zhrrd015.prog.abap:108` `IF password = '<4자>'`(HR 리포트 접근코드, 현재 리포트만 되고 마스킹 안 됨) | 🧑 판단: (a) 리포트만 유지 (b) `hardcoded_secret`을 마스킹 대상으로 승격 — 현 `maskValue`는 숫자만 가리므로 문자 비밀번호용 마스커 추가 필요 | 🧑 판단 → 🤖 | 다음 스냅샷 pii-report `hardcoded_secret` 1건 + 판단 기록 |
