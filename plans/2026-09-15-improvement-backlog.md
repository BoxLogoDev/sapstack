...
| 6   | 배포 전 시크릿 25건 검토           | ✅ 열람(09-15): 25건 중 24건은 주석 처리된 `PASSWORD = ' '` 자리표시자(FTP/RFC 호출 블록) — 오탐. `scrub.mjs` 규칙에서 공백 리터럴 제외 + 테스트 추가 → 재스캔 1건(09-15 13:14 스냅샷은 수정 전 코드로 돌아 25건 표기, 새 규칙은 09-16 06:30부터). 남은 1건 `zhr0/zhrrd015.prog.abap:108` `IF password = '<4자>'`(HR 리포트 접근코드, 현재 리포트만 되고 마스킹 안 됨) | 🧑 판단: (a) 리포트만 유지 (b) `hardcoded_secret`을 마스킹 대상으로 승격 — 현 `maskValue`는 숫자만 가리므로 문자 비밀번호용 마스커 추가 필요 | 🧑 판단 → 🤖 | 다음 스냅샷 pii-report `hardcoded_secret` 1건 + 판단 기록 |
