...
- **#6 시크릿 1건(HR 리포트 접근코드)**: 09-15 사용자 결정으로 마스킹 승격(scrub 규칙, `'***'`). 09-15 오후 재수집은 마무리 단계가 외부 Ctrl+C(0xC000013A)로 중단, 09-16 06:36 예약 실행은 사내망 밖이라 0건 → 직전 스냅샷 복원(HEAD 7992688d 미마스킹). 09-16 09:27 수동 재수집 진행 — 커밋 후 `git show HEAD:src/zhr0/zhrrd015.prog.abap | sed -n 108p` 로 `***` 확인
