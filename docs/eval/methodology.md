...
- **유료/로컬**: `eval-diagnosis.sh` 실제 채점 — Anthropic API 호출. **per-PR/CI 자동 실행 안 함.**
  개발자가 의도적으로 로컬에서 실행한다. `ANTHROPIC_API_KEY` 없으면 자동 dry-run 전환.
...
# 3. 실제 채점 (ANTHROPIC_API_KEY 필요, 로컬)
export ANTHROPIC_API_KEY=sk-...
./scripts/eval-diagnosis.sh --module FI         # FI 만
./scripts/eval-diagnosis.sh --all --limit 5     # 앞 5건
```
