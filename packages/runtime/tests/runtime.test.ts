...
test("catalog and knowledge services use the canonical repository assets", async t => {
  ...
  // asset-manifest.json is regenerated by scripts/build-multi-ai.sh --write; the catalog must agree with it
  const { counts } = await new FileSystemAssetProvider(repositoryRoot).manifest();
  assert.equal((await runtime.catalog.plugins()).length, counts.plugins);
  assert.equal((await runtime.catalog.agents()).length, counts.agents);
  assert.equal((await runtime.catalog.commands()).length, counts.commands);
